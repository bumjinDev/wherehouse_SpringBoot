...
**기존 매물 추천 기능과 저장소 구조가 다른 이유.** 매물 추천은 전체 매물 풀에서 다차원 조건(가격, 면적, 안전 점수)으로 필터링과 정렬을 수행하는 읽기 집약적 기능이며, 배치 파이프라인으로 적재된 대량 데이터를 대상으로 한다. 이 특성상 읽기 저장소를 통한 최적화가 효과적이다. 반면 방문 예약의 슬롯 조회(F003)는 특정 매물 한 건에 속한 소수의 슬롯을 조회하는 것이며, 결과 집합의 크기가 작다. 색인이 설정된 Oracle 직접 조회로 충분한 응답 시간을 달성할 수 있으므로, 별도의 읽기 저장소를 두는 복잡성을 도입하지 않는다.
...
예약 요청(F004)은 탐색자가 결과를 즉시 인지할 수 있는 시간 내에 확정 또는 거부 결과를 반환해야 한다. 슬롯 조회(F003)는 매물 상세 화면의 응답 시간에 포함되므로, 기존 매물 상세 조회와 동등한 수준의 응답 시간을 유지해야 한다. F003은 특정 매물 한 건에 속한 소수의 슬롯을 조회하며, 적절한 색인(섹션 4.2)이 설정된 Oracle 직접 조회로 이 목표를 달성한다. 구체적 목표 응답 시간과 동시 처리량 목표는 구현 단계의 측정 계획에서 확정한다.
...
본 기능은 5개의 신규 테이블을 추가한다. 모든 테이블의 기본 키는 Oracle 시퀀스로 생성되는 순번이다.

#### 4.1.1 VISIT_WINDOW

등록자가 공개하는 방문 가능 시간 윈도우를 저장한다. 하나의 매물에 여러 윈도우가 존재할 수 있으며, 각 윈도우는 고정 길이 슬롯으로 분할된다.
...
윈도우를 분할하여 생성된 고정 길이 방문 슬롯을 저장한다. 각 슬롯은 예약의 대상이 되는 단위 자원이다.
...
탐색자의 방문 예약을 저장한다. 하나의 슬롯에는 최대 하나의 확정 예약만 존재할 수 있다.
...
조회 성능을 위해 다음 색인을 정의한다.

| 테이블 | 색인명 | 대상 컬럼 | 용도 |
|--------|--------|----------|------|
| VISIT_SLOT | IX_SLOT_PROPERTY | (PROPERTY_ID, LEASE_TYPE, STATUS) | F003: 매물별 슬롯 조회 |
| VISIT_SLOT | IX_SLOT_WINDOW | (WINDOW_ID) | F002: 윈도우 철회 시 소속 슬롯 일괄 조회 |
| VISIT_SLOT | IX_SLOT_STATUS_TIME | (STATUS, END_TIME) | F007: 종료 대상 슬롯 주기적 식별 |
| VISIT_RESERVATION | IX_RESV_SEARCHER | (SEARCHER_USER_ID, STATUS) | F008: 탐색자 예약 현황 조회 및 시간 겹침 검증 |
| VISIT_RESERVATION | IX_RESV_SLOT | (SLOT_ID, STATUS) | F004: 슬롯의 확정 예약 존재 여부 확인 |
| VISIT_RESERVATION | IX_RESV_PROPERTY_SEARCHER | (PROPERTY_ID, SEARCHER_USER_ID, STATUS) | F004: 동일 매물 중복 예약 검증 |
| REOPEN_SUBSCRIPTION | IX_SUB_SLOT | (SLOT_ID, STATUS) | F005/F006: 재개방 알림 대상 구독자 조회 |
| VISIT_NOTIFICATION | IX_NOTIF_USER | (USER_ID, IS_READ, CREATED_AT) | 알림 조회 |
...
등록자가 자신이 등록한 매물에 대해 방문 가능 시간 윈도우를 공개한다. 공개된 윈도우는 고정 길이 슬롯으로 분할되어 각 슬롯이 예약 가능 상태로 생성된다. 이 기능은 방문 예약의 공급 측을 형성하며, 이후 모든 기능의 대상이 되는 슬롯을 생산한다.

#### 처리 흐름

**1단계: 요청 검증.** 방문 예약 API 진입점이 요청을 수신한다. 필수 입력값(대상 매물 식별자, 임대 유형, 윈도우 시작 시각, 종료 시각)의 형식과 존재 여부를 검증한다. 슬롯 분할 단위가 미지정이면 기본값 30분을 적용한다. 윈도우 종료 시각이 시작 시각보다 이전이거나, 시작 시각이 현재 시각보다 이전이면 거부한다.

**2단계: 매물 유효성 확인.** 방문 예약 쓰기 서비스 계층이 대상 매물의 존재와 상태를 확인한다. (PROPERTY_ID, LEASE_TYPE) 조합으로 해당 매물 테이블에서 매물을 조회한다. 매물이 존재하지 않거나, STATUS가 ACTIVE가 아니거나, REGISTERED_USER_ID가 존재하지 않으면 거부한다.

**3단계: 등록자 권한 확인.** 요청자의 식별자와 매물의 REGISTERED_USER_ID가 일치하는지 확인한다. 불일치하면 권한 없음으로 거부한다.

**4단계: 윈도우 시간 겹침 검증.** 동일 매물(PROPERTY_ID, LEASE_TYPE)의 활성 윈도우(STATUS = ACTIVE) 중 요청된 시간 범위와 겹치는 윈도우가 존재하는지 확인한다. 겹치는 윈도우가 있으면 거부한다. 이 규칙은 같은 시간대의 슬롯이 중복 생성되는 것을 방지한다.

**5단계: 윈도우 저장 및 슬롯 생성.** 윈도우 레코드를 Oracle에 저장한 뒤, 윈도우의 시간 범위를 슬롯 분할 단위에 따라 고정 길이 슬롯으로 분할한다. 각 슬롯은 시작 시각, 종료 시각, STATUS = AVAILABLE로 생성된다. 윈도우 저장과 슬롯 일괄 생성은 하나의 원자적 실행 단위 안에서 수행되어, 윈도우는 저장되었으나 슬롯이 생성되지 않은 부분 실패가 발생하지 않도록 한다.

**6단계: 응답 반환.** 생성된 윈도우 정보와 슬롯 목록을 응답으로 반환한다.

### 6.2 F002 — 방문 윈도우 철회

#### 개요

등록자가 이미 공개한 방문 윈도우를 철회한다. 철회된 윈도우의 슬롯은 더 이상 예약 대상이 아니며, 확정된 예약이 있으면 무효화되고 영향받은 탐색자에게 통지된다. 윈도우 철회는 되돌릴 수 없다.

#### 처리 흐름

**1단계: 요청 검증 및 권한 확인.** 방문 예약 API 진입점이 윈도우 식별자와 요청자 식별자를 수신한다. 해당 윈도우가 존재하는지, STATUS가 ACTIVE인지, 요청자가 윈도우의 REGISTRANT_USER_ID와 일치하는지 확인한다. 어느 하나라도 실패하면 거부한다.

**2단계: 확정 예약 식별.** 해당 윈도우에 소속된 슬롯 중 STATUS가 RESERVED인 슬롯을 조회하고, 그 슬롯에 연결된 확정 예약(STATUS = CONFIRMED)을 식별한다.

**3단계: 원자적 상태 전이.** 하나의 원자적 실행 단위 안에서 다음을 수행한다. 윈도우의 STATUS를 WITHDRAWN으로, WITHDRAWN_AT을 현재 시각으로 갱신한다. 해당 윈도우의 모든 활성 슬롯(AVAILABLE, RESERVED)의 STATUS를 WITHDRAWN으로 전이한다(T-S3, T-S6). 식별된 확정 예약의 STATUS를 INVALIDATED로, INVALIDATED_AT을 현재 시각으로 갱신한다(T-R2). 해당 슬롯에 연결된 활성 구독(REOPEN_SUBSCRIPTION의 STATUS = ACTIVE)을 EXPIRED로 전이하고 TERMINATION_REASON을 SLOT_CLOSED로 설정한다.

**이 구간은 경합 구간이다.** 윈도우 철회와 동시에 소속 슬롯에 대한 예약 요청(F004)이 도착할 수 있다. 이 경합의 요구조건은 섹션 8에서 정의하며, 해결 방식은 본 설계에서 결정하지 않는다.

**4단계: 알림 생성.** 무효화된 각 예약의 탐색자에게 RESERVATION_INVALIDATED 유형의 알림을 생성한다.

**5단계: 응답 반환.** 철회 처리 결과와 무효화된 예약의 목록을 응답으로 반환한다.

### 6.3 F003 — 방문 슬롯 조회

#### 개요

탐색자가 특정 매물에 공개된 방문 슬롯과 그 예약 가능 여부를 조회한다. 이 기능은 매물 상세 화면에서 탐색자가 예약할 슬롯을 선택하는 진입점이다.

#### 처리 흐름

**1단계: 요청 수신.** 방문 예약 API 진입점이 매물 식별자와 임대 유형을 수신한다. 이 경로는 인증이 불필요하며 모든 이용자가 접근할 수 있다. 인증 정보가 존재하면 추출하되, 부재 시에도 접근을 허용한다(섹션 3.1 참조).

**2단계: 슬롯 목록 조회.** 방문 예약 조회 서비스 계층이 Oracle에서 해당 매물(PROPERTY_ID, LEASE_TYPE)의 활성 슬롯(STATUS가 AVAILABLE 또는 RESERVED)을 시작 시각 순으로 조회한다. IX_SLOT_PROPERTY 색인(섹션 4.2)이 이 조회를 지원한다.

**3단계: 응답 구성.** 각 슬롯의 시작 시각, 종료 시각, 현재 상태(예약 가능 또는 예약됨)를 포함하는 슬롯 목록을 구성한다. 슬롯을 예약한 탐색자의 신원은 응답에 포함되지 않는다. 종료되거나 철회된 슬롯은 조회 대상에서 제외되므로 응답에 포함되지 않는다.

**조회와 예약 사이의 상태 변경에 대한 고려.** 이 조회와 실제 예약 시도(F004) 사이에 다른 탐색자의 예약이 확정되어 슬롯 상태가 변할 수 있다. 조회에서 예약 가능으로 확인한 슬롯이 예약 시도 시점에 이미 확정되어 있으면, F004의 거부 응답으로 처리된다.
...
**1단계: 요청 검증.** 방문 예약 API 진입점이 슬롯 식별자와 요청자(탐색자) 식별자를 수신한다. 인증이 필수이며, 인증되지 않은 요청은 거부한다.

**2단계: 슬롯 유효성 확인.** 해당 슬롯이 존재하는지, STATUS가 AVAILABLE인지 확인한다. AVAILABLE이 아니면 슬롯이 이미 확정되었다는 사유와 함께 같은 매물의 현재 예약 가능한 슬롯 목록을 반환하며 거부한다.

**3단계: 자기 매물 예약 제한.** 해당 슬롯이 속한 매물의 REGISTERED_USER_ID와 요청자의 식별자가 일치하는지 확인한다. 일치하면 자기 매물 예약 불가 사유로 거부한다.

**4단계: 동일 매물 중복 예약 검증.** 요청자가 같은 매물(PROPERTY_ID, LEASE_TYPE)에 대해 이미 활성 예약(STATUS = CONFIRMED)을 가지고 있는지 확인한다. 존재하면 동일 매물 중복 예약 사유로 거부한다.

**5단계: 시간 겹침 검증.** 요청자의 기존 활성 예약 목록에서, 요청된 슬롯의 방문 시간 구간과 겹치는 예약이 존재하는지 확인한다. 두 시간 구간이 겹치는 조건은 "기존 예약의 시작 시각 < 요청 슬롯의 종료 시각"이고 "기존 예약의 종료 시각 > 요청 슬롯의 시작 시각"이다. 겹치는 예약이 존재하면 시간 겹침 사유로 거부한다.

**6단계: 슬롯 상태 전이 및 예약 생성.** 슬롯의 STATUS를 AVAILABLE에서 RESERVED로 전이(T-S1)하고, 예약 레코드를 STATUS = CONFIRMED로 생성한다. 슬롯 상태 전이와 예약 생성은 하나의 원자적 실행 단위 안에서 수행된다.

**이 구간은 본 기능의 핵심 경합 구간이다.** 여러 탐색자의 동시 예약 요청이 같은 슬롯에 도달할 때, 2단계의 AVAILABLE 확인과 6단계의 RESERVED 전이 사이에 경합이 발생한다. 이 경합 구간의 위치, 요구조건, 기존 매물 등록과의 차이는 섹션 8에서 상세히 식별한다. 해결 방식은 본 설계에서 결정하지 않는다.

**7단계: 알림 생성.** 확정에 성공하면, 해당 슬롯이 속한 매물의 등록자에게 SLOT_RESERVED 유형의 알림을 생성한다.

**8단계: 응답 반환.**

확정에 성공한 탐색자에게는 다음을 반환한다. 예약 식별자, 대상 슬롯의 방문 시각, 대상 매물 정보, 그리고 확정 시점에 공개되는 등록자의 연락 정보. 연락 정보는 등록자의 닉네임과 회원 프로필에서 제공하는 연락 경로를 포함한다. 이 정보는 예약 확정 시점에 확정된 한 쌍의 당사자에게만 공개되며, 예약 이전에는 노출되지 않는다.

확정되지 못한 탐색자에게는 거부 사유와 같은 매물의 현재 예약 가능한 슬롯 목록을 반환한다.

### 6.5 F005 — 방문 예약 취소

#### 개요

탐색자가 자신의 활성 예약을 취소한다. 취소된 예약의 슬롯은 다시 예약 가능 상태가 되어 다른 탐색자가 예약할 수 있다.

#### 처리 흐름

**1단계: 요청 검증 및 권한 확인.** 방문 예약 API 진입점이 예약 식별자와 요청자 식별자를 수신한다. 해당 예약이 존재하는지, STATUS가 CONFIRMED인지, 요청자가 예약의 SEARCHER_USER_ID와 일치하는지 확인한다. 어느 하나라도 실패하면 거부한다.

**2단계: 취소 가능 시점 검증.** 해당 예약이 연결된 슬롯의 시작 시각이 현재 시각 이후인지 확인한다. 슬롯 시작 시각이 이미 경과했으면 취소 불가로 거부한다. 기획서에 따라 시작 시간 이전까지는 별도의 취소 마감 시점 없이 취소를 허용한다.

**3단계: 원자적 상태 전이.** 하나의 원자적 실행 단위 안에서 다음을 수행한다. 예약의 STATUS를 CANCELLED로, CANCELLED_AT을 현재 시각으로 갱신한다(T-R1). 해당 슬롯의 STATUS를 RESERVED에서 AVAILABLE로 전이한다(T-S4).

**이 구간은 경합 구간이다.** 슬롯이 AVAILABLE로 돌아가는 시점에 다른 탐색자의 예약 요청(F004)이 동시에 도착할 수 있다. 이 경합의 요구조건은 섹션 8에서 정의한다.

**4단계: 재개방 알림 발송.** 해당 슬롯에 연결된 활성 구독(REOPEN_SUBSCRIPTION의 STATUS = ACTIVE)의 구독자에게 SLOT_REOPENED 유형의 알림을 생성한다. 구독 자체는 종료하지 않는다. 기획서에 따라 슬롯이 다시 열릴 때마다 알림이 발송되며, 종료 조건에 도달하기 전까지 구독은 유지된다.
...
탐색자가 이미 예약된 슬롯이 취소로 다시 열릴 경우 알림을 받기 위해 구독을 신청하거나 해제한다. 구독은 어떤 예약도 보장하지 않으며, 슬롯이 다시 열릴 때 구독자에게 알림을 전달하는 알림 신청이다. 시스템은 슬롯을 자동으로 배정하지 않는다.

#### 처리 흐름 — 구독 신청

**1단계: 요청 검증.** 방문 예약 API 진입점이 슬롯 식별자와 요청자 식별자를 수신한다. 해당 슬롯이 존재하는지, STATUS가 RESERVED인지 확인한다. AVAILABLE 상태의 슬롯에 대한 구독은 무의미하므로 거부한다. CLOSED 또는 WITHDRAWN 상태의 슬롯에 대한 구독도 거부한다.

**2단계: 중복 구독 검증.** 요청자가 해당 슬롯에 이미 활성 구독(STATUS = ACTIVE)을 가지고 있는지 확인한다. 이미 구독 중이면 중복 구독 사유로 거부한다.

**3단계: 구독 생성.** 구독 레코드를 STATUS = ACTIVE로 Oracle에 저장한다.

**4단계: 응답 반환.** 구독 신청 결과를 반환한다.

#### 처리 흐름 — 구독 해제

**1단계: 요청 검증 및 권한 확인.** 요청자가 해당 슬롯에 대해 활성 구독을 가지고 있는지 확인한다. 활성 구독이 없으면 거부한다.

**2단계: 구독 종료.** 해당 구독의 STATUS를 CANCELLED로, TERMINATED_AT을 현재 시각으로, TERMINATION_REASON을 USER_CANCELLED로 갱신한다.

**3단계: 응답 반환.** 구독 해제 결과를 반환한다.

#### 구독 종료 조건

기획서에 따라 구독은 다음 세 경우에 종료된다. 이 처리는 F006 자체가 아니라 각각의 촉발 기능에서 수행된다.

| 종료 조건 | 새 상태 | TERMINATION_REASON | 처리 주체 |
|-----------|---------|-------------------|----------|
| 슬롯이 재개방 없이 종료 상태에 도달 | EXPIRED | SLOT_CLOSED | F007(슬롯 종료 처리) |
| 구독자가 해당 슬롯을 다시 예약하는 데 성공 | FULFILLED | RESERVED | F004(슬롯 예약) |
| 탐색자가 직접 구독을 해제 | CANCELLED | USER_CANCELLED | F006(구독 해제) |

### 6.7 F007 — 슬롯 종료 및 방문 결과 분류

#### 개요

이 기능은 두 부분으로 구성된다. 첫째, 슬롯의 예정 시간이 경과하면 시스템이 해당 슬롯을 종료 상태로 전환한다. 둘째, 등록자가 종료된 예약의 방문 결과를 분류한다.

#### 처리 흐름 — 슬롯 종료 (시스템 주도)

슬롯 종료 처리 컴포넌트가 1분 주기로 다음을 수행한다.

**1단계: 종료 대상 식별.** Oracle에서 STATUS가 AVAILABLE 또는 RESERVED이면서 END_TIME이 현재 시각 이전인 슬롯을 조회한다.

**2단계: 상태 전이.** 식별된 각 슬롯의 STATUS를 CLOSED로 전이한다(T-S2 또는 T-S5). 슬롯에 연결된 확정 예약(STATUS = CONFIRMED)이 있으면 해당 예약의 STATUS를 COMPLETED로 전이한다(T-R3). VISIT_RESULT는 NULL로 남겨 등록자의 분류를 기다린다.

**3단계: 구독 폐기.** 종료된 슬롯에 연결된 활성 구독을 EXPIRED로 전이하고 TERMINATION_REASON을 SLOT_CLOSED로 설정한다.

#### 처리 흐름 — 방문 결과 분류 (등록자 주도)

**1단계: 요청 검증 및 권한 확인.** 방문 예약 API 진입점이 예약 식별자, 분류 값(VISITED 또는 NO_SHOW), 요청자 식별자를 수신한다. 해당 예약이 존재하는지, STATUS가 COMPLETED인지, VISIT_RESULT가 아직 NULL인지 확인한다. 해당 예약이 연결된 슬롯의 매물에 대해 요청자가 등록자인지 확인한다.

**2단계: 결과 분류.** 예약의 VISIT_RESULT를 요청된 분류 값으로, RESULT_CLASSIFIED_AT을 현재 시각으로 갱신한다.

**3단계: 응답 반환.** 분류 결과를 반환한다.

**분류 마감에 대한 고려.** 기획서에 따라 방문 결과 분류에는 별도의 마감 시점이 없다. 등록자가 분류하지 않은 예약은 종료(미분류)로 유지된다.

### 6.8 F008 — 예약 및 슬롯 현황 조회

#### 개요

탐색자가 자신의 예약과 구독 현황을 조회하고, 등록자가 자기 매물의 슬롯과 예약 현황을 조회한다. 이 기능은 예약 이후 시간에 걸친 상태 가시성을 제공한다. 관리 화면의 정확성을 위해 Oracle에서 직접 조회한다.

#### 처리 흐름 — 탐색자 예약 현황 조회

**1단계: 요청 수신.** 인증된 탐색자의 식별자를 기준으로 조회한다.

**2단계: 예약 목록 조회.** Oracle에서 요청자의 SEARCHER_USER_ID에 해당하는 예약을 조회한다. 활성 예약(STATUS = CONFIRMED)과 종료된 예약(CANCELLED, INVALIDATED, COMPLETED)을 모두 포함하며, 최신순으로 정렬한다. 각 예약에 연결된 슬롯의 시간 정보와 매물 정보를 함께 반환한다.

**3단계: 응답 구성.** 다른 탐색자의 예약은 포함되지 않는다. 확정된 예약의 등록자 연락 정보는 해당 예약이 CONFIRMED 또는 COMPLETED 상태일 때만 포함된다.

#### 처리 흐름 — 탐색자 구독 현황 조회

Oracle에서 요청자의 SEARCHER_USER_ID에 해당하는 구독을 조회한다. 활성 구독(STATUS = ACTIVE)과 종료된 구독을 모두 포함한다. 각 구독에 연결된 슬롯의 시간 정보와 현재 상태를 함께 반환한다.

#### 처리 흐름 — 등록자 슬롯 현황 조회

**1단계: 요청 수신.** 인증된 등록자의 식별자와 대상 매물 식별자를 수신한다.

**2단계: 권한 확인.** 해당 매물의 REGISTERED_USER_ID와 요청자의 식별자가 일치하는지 확인한다.

**3단계: 슬롯 목록 조회.** Oracle에서 해당 매물(PROPERTY_ID, LEASE_TYPE)의 모든 슬롯을 조회한다. 각 슬롯의 시작 시각, 종료 시각, 상태, 그리고 슬롯에 연결된 예약 정보(예약자 닉네임, 예약 상태, 방문 결과)를 포함한다.

**4단계: 응답 구성.** 다른 등록자의 매물 슬롯은 포함되지 않는다. 확정 예약의 탐색자 연락 정보는 해당 예약이 CONFIRMED 또는 COMPLETED 상태일 때만 포함된다.
...
| lease_type | String | 임대 유형 |
| slots | Array | 활성 슬롯 목록 (시간순) |
| slots[].slot_id | Long | 슬롯 식별자 |
| slots[].start_time | String | 슬롯 시작 시각 |
| slots[].end_time | String | 슬롯 종료 시각 |
| slots[].status | String | 슬롯 상태 (AVAILABLE 또는 RESERVED) |
