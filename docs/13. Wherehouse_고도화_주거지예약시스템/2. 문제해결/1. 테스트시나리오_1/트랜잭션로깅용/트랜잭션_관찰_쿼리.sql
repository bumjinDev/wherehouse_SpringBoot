...
-- ════════════════════════════════════════════════════════════════════════════
-- B. 현재 잡힌 락 (V$LOCK) — 메인 폴링 쿼리 (테이블명 포함)
-- ════════════════════════════════════════════════════════════════════════════
...
-- CTIME:   현재 락을 잡고 있은 시간 (초)
--
-- LOCKED_TABLE 컬럼 의미:
--   TM 락: V$LOCK.ID1 = OBJECT_ID 이므로 USER_OBJECTS 와 직접 조인.
--   TX 락: 객체 정보가 없지만, 같은 SID 가 잡은 TM 락에서 간접 추론.
--          (한 트랜잭션이 TX 1건 + 영향 테이블별 TM 1건씩 잡는다.)
SELECT l.SID,
       s.SERIAL#,
       s.USERNAME,
       l.TYPE,
       DECODE(l.LMODE,
              ...
              4,'Share', 5,'S/Row-X(SSX)', 6,'Exclusive(X)', TO_CHAR(l.LMODE))      AS LMODE_DESC,
       DECODE(l.REQUEST,
              0,'-', 1,'Null', 2,'Row-S(SS)', 3,'Row-X(SX)',
              4,'Share', 5,'S/Row-X(SSX)', 6,'Exclusive(X)', TO_CHAR(l.REQUEST))    AS REQUEST_DESC,
       l.CTIME      AS HELD_SECONDS,
       l.BLOCK,
       -- TM 락은 직접, TX 락은 같은 세션의 TM 락들을 LISTAGG 로 묶어 표시
       CASE
           WHEN l.TYPE = 'TM' THEN (SELECT o.OBJECT_NAME
                                      FROM USER_OBJECTS o
                                     WHERE o.OBJECT_ID = l.ID1)
           WHEN l.TYPE = 'TX' THEN (SELECT LISTAGG(o.OBJECT_NAME, ', ')
                                           WITHIN GROUP (ORDER BY o.OBJECT_NAME)
                                      FROM V$LOCK l2
                                      JOIN USER_OBJECTS o ON o.OBJECT_ID = l2.ID1
                                     WHERE l2.SID = l.SID
                                       AND l2.TYPE = 'TM')
           ELSE NULL
       END                                          AS LOCKED_TABLE,
 ...
 WHERE s.USERNAME = 'SCOTT'
   AND l.TYPE IN ('TX','TM')
   -- 특정 테이블만 보고 싶다면 아래 주석 해제 (TM 직접 + TX 의 보유 테이블 매칭):
   -- AND (
   --     (l.TYPE = 'TM' AND EXISTS (SELECT 1 FROM USER_OBJECTS o
   --                                 WHERE o.OBJECT_ID = l.ID1
   --                                   AND o.OBJECT_NAME IN ('VISIT_SLOT','VISIT_RESERVATION')))
   --  OR (l.TYPE = 'TX' AND EXISTS (SELECT 1 FROM V$LOCK l2
   --                                  JOIN USER_OBJECTS o ON o.OBJECT_ID = l2.ID1
   --                                 WHERE l2.SID = l.SID AND l2.TYPE = 'TM'
   --                                   AND o.OBJECT_NAME IN ('VISIT_SLOT','VISIT_RESERVATION')))
   -- )
