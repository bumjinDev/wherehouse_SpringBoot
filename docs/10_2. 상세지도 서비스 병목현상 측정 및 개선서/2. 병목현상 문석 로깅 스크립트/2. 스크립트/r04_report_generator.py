...
def create_r04_resultdata_sheet(df: pd.DataFrame, writer):
    ...
    metrics_config = {
        ...
        'avgTaskMs': {
            ...
            'transform': lambda x: round(x / 1_000_000 / 3, 3) if x is not None else None
        },
        ...
        'addressCacheHitRate': {
            ...
            'allow_none': False,
            'transform': lambda x: 100.0 if x is True else 0.0
        },
        'addressExecutionMs': {
        ...
        'arrestRateValue': {
            ...
            'description': '검거율 값',
            'transform': lambda x: round(x, 3) if x is not None else None
        },
