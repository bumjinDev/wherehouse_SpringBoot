...
def create_resultdata_sheet_base(df: pd.DataFrame, 
    ...
    metrics_config 구조:
        {
            'metric_name': {
                ...
                'allow_none': True/False (선택, 기본값 False)
            }
        }
    
    allow_none 설명:
        - False (기본값): null 값은 dropna()로 제거 후 통계 계산
        - True: null 값도 유지하여 transform에 전달 (errorMessage 같은 필드)
    ...
    for metric_name, config in metrics_config.items():
        ...
        values = df_end['resultData'].apply(
            lambda x: extract_nested_field(x, path, allow_none=allow_none) 
            if isinstance(x, dict) else None
        )
        
        # allow_none=False인 경우: null 제거 후 transform
        # allow_none=True인 경우: transform 먼저 적용 후 null 제거
        if not allow_none:
            ...
            # allow_none=True: transform 먼저 적용
            if transform:
                try:
                    values = values.apply(lambda x: transform(x) if x is not None or pd.isna(x) else x)
                except Exception as e:
                    print(f"  Warning: {sheet_name} - '{metric_name}' 변환 실패: {e}")
                    continue
            ...
            if len(values) == 0:
                print(f"  Warning: {sheet_name} - '{metric_name}' 필드를 찾을 수 없거나 모두 null입니다 (path: {path})")
                continue
...
def extract_nested_field(data: Dict, path: str, allow_none: bool = False) -> Any:
    """
    중첩된 딕셔너리에서 필드 추출
    ...
    Args:
        ...
        allow_none: True면 null 값도 유효한 값으로 반환, False면 None으로 통일
    
    Returns:
        추출된 값 또는 None
    
    allow_none 설명:
        - False (기본값): 필드가 없거나 null이면 모두 None 반환 → dropna()로 제거됨
        - True: 필드가 있고 값이 null이면 null 반환 (dropna 전에 transform으로 처리)
                필드 자체가 없으면 None 반환 → dropna()로 제거됨
    ...
    Example:
        ...
        123
        >>> data = {'error': None}
        >>> extract_nested_field(data, 'error', allow_none=False)
        None  # dropna()로 제거됨
        >>> extract_nested_field(data, 'error', allow_none=True)
        None  # 유지됨 (transform에서 처리)
        >>> extract_nested_field(data, 'missing', allow_none=True)
        None  # 필드 없음 → dropna()로 제거됨
    """
    ...
    # allow_none=True이고 필드는 존재하지만 null인 경우 → null 반환
    # allow_none=False이고 value가 None이면 → None 반환 (dropna로 제거)
    if allow_none:
        return value  # null도 반환
    else:
