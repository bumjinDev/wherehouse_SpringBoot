# 단란주점 데이터 Oracle DB 설계 및 JPA 구현

## 1. Oracle 테이블 DDL

```sql
CREATE TABLE DANRAN_BARS (
    ...
    CONSTRAINT UK_DANRAN_MGMT_NUM UNIQUE (MANAGEMENT_NUMBER)
);

CREATE SEQUENCE SEQ_DANRAN_BARS START WITH 1 INCREMENT BY 1;
```
...
@Entity
@Table(name = "DANRAN_BARS")
@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class DanranBars {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "danran_seq")
    @SequenceGenerator(name = "danran_seq", sequenceName = "SEQ_DANRAN_BARS", allocationSize = 1)
    @Column(name = "ID")
    private Long id;
...
import com.wherehouse.safety.entity.KaraokeBars;
...
import com.wherehouse.safety.entity.KaraokeBars;
import com.wherehouse.safety.repository.KaraokeBarsRepository;
...
@Slf4j
public class DanranBarsDataLoader implements CommandLineRunner {

    private final DanranBarsRepository danranBarsRepository;
    private final ResourceLoader resourceLoader;

    @Value("${app.csv.danran-bars-data-path}")
    private String csvFilePath;

    @Override
    @Transactional
    public void run(String... args) {
        if (danranBarsRepository.count() > 0) {
            log.info("단란주점 데이터 이미 존재. 로딩 스킵");
            return;
        }
        ...
        try {
            ...
            try (CSVReader reader = new CSVReader(new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8))) {
                ...
                log.info("단란주점 데이터 로딩 시작 - 총 {} 건", totalRows);
                ...
                for (int i = 1; i < csvData.size(); i++) {
                    ...
                    if (row.length >= 43) {
                        try {
                            DanranBars bar = DanranBars.builder()
                ...
                log.info("단란주점 데이터 로딩 완료 - 성공: {}건, 실패: {}건, 전체: {}건",
...
파일 위치: `src/main/resources/data/단란주점_데이터.csv`