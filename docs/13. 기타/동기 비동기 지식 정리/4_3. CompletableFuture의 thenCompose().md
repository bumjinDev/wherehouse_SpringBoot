...
이러한 타입 중첩은 단순한 문법적 불편함이 아니다. 실제 Payment 객체에 접근하려면 세 번의 join()을 호출해야 한다. 각 중첩 레벨에서 발생하는 예외를 추적하고 처리하기 위해서는 각 레벨의 CompletableFuture를 개별적으로 다뤄야 하며, 어느 레벨에서 발생한 예외인지 파악하기 어렵다. 
...
### 3.1 CompletionStage 제약을 통한 의도 명시
