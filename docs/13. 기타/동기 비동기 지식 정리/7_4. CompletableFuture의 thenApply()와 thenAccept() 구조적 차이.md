...
CompletableFuture의 메서드 체계는 **수십 년간 축적된 함수형 프로그래밍과 비동기 시스템 설계의 지혜**를 Java 생태계에 적용한 산물이다. thenApply()와 thenAccept()의 구분은 임의적 선택이 아니라, 수학적 엄밀성과 실용적 요구사항의 교집합에서 도출된 필연적 설계 결과물이다.

널리 퍼진 오해들은 표면적 관찰에 기반한 불완전한 모델이다. 진정한 이해는 내부 메커니즘과 설계 철학을 파악할 때 비로소 도달한다. 이 문서가 그러한 이해의 시작점이 될 것이다.