...
**의도 1: 지역성(Locality) 최적화**

Worker가 자신이 방금 생성한 작업을 즉시 처리하면, 해당 작업의 데이터가 CPU 캐시에 남아있을 가능성이 높다. LIFO 순서로 처리하면 캐시 히트율이 증가한다.

구체적 메커니즘:
- Worker가 Task A를 실행 중 → Task A의 데이터가 L1/L2 캐시에 로드됨
- Task A가 Task B, C를 생성 (fork) → Task B, C는 deque head에 push
- Worker가 Task C를 즉시 처리 (head에서 pop) → Task A와 Task C가 관련 데이터를 공유하면 캐시 히트
...
### 2.7 Per-Worker Deque의 설계 의도: 락 경쟁 최소화와 동시 접근 분리

#### 2.7.1 공유 큐의 문제점 (ThreadPoolExecutor)

**단일 공유 큐의 동시 접근:**

```java
// ThreadPoolExecutor의 단일 공유 큐
...
class Worker {
    void run() {
        while (true) {
            Runnable task = workQueue.take(); // 여기서 락 획득 필요
            task.run();
        }
    }
}
...
```
시간 T:
Worker 1: workQueue.take() 시도 → 큐 락 획득
Worker 2: workQueue.take() 시도 → Worker 1이 락 해제할 때까지 블로킹
Worker 3: workQueue.take() 시도 → 대기
Worker 4: workQueue.take() 시도 → 대기

메인 스레드: workQueue.put(task) 시도 → 락 대기

결과: 4코어 CPU에서 실제로는 1개만 큐에 접근 가능, 나머지 대기
```

CPU-bound 작업에서 작업 하나의 실행 시간이 매우 짧으면 (예: 1ms), 큐 접근 락 경쟁이 심각한 병목이 된다:

```
작업 실행: 1ms
큐 락 획득 대기: 0.5ms
→ 실제 CPU 활용률: 1 / 1.5 = 66.7%
```

#### 2.7.2 ForkJoinPool의 Per-Worker Deque 설계

```
각 Worker가 독립된 deque 소유:

Worker 1 Deque          Worker 2 Deque          Worker 3 Deque
┌──────────┐            ┌──────────┐            ┌──────────┐
│ Task A   │← head      │ Task E   │← head      │ Task H   │← head
│ Task B   │            │ Task F   │            │ Task I   │
│ Task C   │            │ Task G   │            │ Task J   │
│ Task D   │← tail      │          │← tail      │          │← tail
└──────────┘            └──────────┘            └──────────┘
    ↓                        ↑                        ↑
Worker 1이 head에서       Worker 3이 tail에서      Worker 4가 tail에서
push/pop (LIFO)          steal (FIFO)            steal (FIFO)
락 불필요!               CAS만으로 충분           CAS만으로 충분
```
...
```
시간 T:
Worker 1: 자신의 deque head에서 pop → 락 불필요 (다른 Worker는 tail만 접근)
Worker 2: 자신의 deque head에서 pop → 락 불필요
Worker 3: Worker 1의 deque tail에서 steal → CAS(Compare-And-Swap)만 필요
Worker 4: Worker 2의 deque tail에서 steal → CAS만 필요

동시에 4개 Worker가 모두 작업 가져오기 가능!
락 경쟁 거의 없음 (tail 접근 시에만 CAS로 충돌 처리)
```

#### 2.7.3 CAS의 효율성

**의사 코드: deque에서 steal할 때**

```java
Task steal() {
    while (true) {
        Task task = deque.peekTail();
        if (task == null) return null;
        
        // CAS: tail이 여전히 task를 가리키면 제거, 아니면 재시도
        if (compareAndSwapTail(task, task.next)) {
            return task; // 성공
        }
        // 실패 시 루프 재시도 (다른 Worker가 먼저 가져갔음)
    }
}
```

CAS는 락보다 훨씬 가볍다:
- 락: 커널 영역 진입, 컨텍스트 스위칭 가능
- CAS: CPU 명령어 1개, 유저 영역에서 완료

#### 2.7.4 정량적 비교: 1만 개의 작은 CPU-bound 작업 처리

**시나리오**: 각 작업이 0.1ms CPU 시간 소요

**ThreadPoolExecutor(4 스레드, 공유 큐):**

```
작업 처리 시간: 0.1ms
큐 락 대기 시간 (평균): 0.05ms (4개 Worker가 경쟁)
총 시간 = 10,000 × 0.15ms / 4 = 375ms
실제 CPU 활용률 = 0.1 / 0.15 = 66.7%
```

**ForkJoinPool(4 스레드, per-worker deque):**

```
작업 처리 시간: 0.1ms
deque 접근 시간: 0.001ms (락 없음, 가끔 CAS 충돌)
총 시간 = 10,000 × 0.101ms / 4 = 252.5ms
실제 CPU 활용률 = 0.1 / 0.101 = 99%
```

**약 1.5배 성능 차이**: 락 경쟁 제거만으로도 상당한 이득

### 2.8 LIFO/FIFO 분리의 설계 의도: 캐시 지역성 최적화

#### 2.8.1 CPU 캐시의 구조적 특성
...
┌──────────┐
│  Core 1  │
│ L1 Cache │ ← 32KB, 접근 시간 ~1ns
│ (64 byte │
│  lines)  │
└────┬─────┘
     │
┌────┴─────┐
│ L2 Cache │ ← 256KB, 접근 시간 ~10ns
└────┬─────┘
     │
┌────┴─────┐
│ L3 Cache │ ← 8MB (공유), 접근 시간 ~40ns
└────┬─────┘
     │
┌────┴─────┐
│   RAM    │ ← 접근 시간 ~100ns
...
```java
// Worker 1이 Task(0, 50M) 처리 시작
Arrays.sort(array, 0, 50_000_000);

// 이 과정에서 array[0] ~ array[50M] 영역이 L1/L2/L3 캐시에 로드됨
// 특히 최근 접근한 부분 (예: array[0] ~ array[1000])은 L1 캐시에 있음
```

#### 2.8.2 LIFO의 캐시 지역성 이점

**Worker가 자신의 작업을 처리하는 순서 (LIFO):**

```
시간 0ms: Worker 1이 Task(0, 50M) 실행
- compute(0, 50M) 시작
- leftTask(0, 25M).fork() → deque에 push
- rightTask(25M, 50M).compute() 즉시 실행

시간 0.1ms: rightTask(25M, 50M) 처리 중
- compute(25M, 50M) 시작
- leftTask(25M, 37.5M).fork() → deque에 push
- rightTask(37.5M, 50M).compute() 즉시 실행

시간 0.2ms: rightTask(37.5M, 50M) 처리 중
- array[37.5M] ~ array[50M] 영역 접근
- 이 데이터가 L1/L2 캐시에 로드됨

시간 0.3ms: rightTask(37.5M, 50M) 완료
- Worker 1의 deque head에서 pop → leftTask(25M, 37.5M) 가져옴

시간 0.4ms: leftTask(25M, 37.5M) 실행
- array[25M] ~ array[37.5M] 영역 접근
- 방금 전에 rightTask에서 array[37.5M] 근처를 접근했으므로
- 일부 데이터가 여전히 L2/L3 캐시에 남아있을 가능성 높음
```

**핵심**: LIFO 순서로 처리하면, 부모 작업과 자식 작업이 접근하는 메모리 영역이 인접하므로, 캐시에 남아있을 가능성이 높다.

#### 2.8.3 FIFO의 큰 작업 우선 steal 이점

**다른 Worker가 작업을 훔칠 때 (FIFO):**

```
Worker 1의 deque 상태:
head → [Task(93.75M, 100M)]  ← 가장 최근에 생성된 작은 작업
       [Task(87.5M, 93.75M)]
       [Task(75M, 87.5M)]
       [Task(50M, 75M)]
tail → [Task(0, 50M)]         ← 가장 먼저 생성된 큰 작업

Worker 3이 steal할 때:
- tail에서 가져감 → Task(0, 50M)
- 이것은 가장 큰 작업 (50M 크기)
```
...
```
Task(0, 50M)을 훔친 Worker 3:
- compute(0, 50M) 실행 → 또 분할
- leftTask(0, 25M).fork()
- rightTask(25M, 50M).compute()
→ Worker 3의 deque에도 작은 작업들이 계속 쌓임
→ 나중에 Worker 4가 Worker 3에서 steal 가능
```

**이유 2: 부하 분산 효과**

```
큰 작업 1개를 steal = 실질적으로 수천 개의 작은 작업을 분담
작은 작업 1개를 steal = 단 1개만 분담 (곧 다시 steal 필요)
```
...
```
큰 작업을 steal하면:
- Worker 3이 한동안 자신의 작업 처리 (자체 deque에서 pop)
- Stealing은 CAS 충돌 위험이 있으므로, 자주 하지 않는 것이 좋음
```
