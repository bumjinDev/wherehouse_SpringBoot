# R-04 비동기 병목 개선: CompletableFuture 실전 구현 가이드

## 전제 지식

본 문서는 CompletableFuture를 활용한 비동기 병목 개선의 **실전 구현 가이드**입니다.

**→ 동기/비동기, 블로킹/논블로킹, OS 스케줄러, Thread 상태, Context Switching 등 모든 이론적 배경은 `0__비동기_프로그래밍과_Thread_기반_병렬화_완전한_기술_분석.md` 문서를 참조하세요.**

**이론 문서에서 다루는 핵심 개념:**
- 블로킹/논블로킹의 정의
- 동기/비동기의 정의
- 4가지 조합 (동기-블로킹, 동기-논블로킹, 비동기-블로킹, 비동기-논블로킹)
- 컴퓨터 아키텍처와 프로그래밍 모델의 진화
- Thread 상태 (NEW, RUNNABLE, WAITING, BLOCKED, TERMINATED)
- OS 스케줄러와 Context Switching
- CPU-bound vs I/O-bound의 차이
- Event Loop, Coroutine 등 다른 비동기 모델

본 문서는 위 이론을 전제로, **카카오 API 15개 병렬 호출** 프로젝트의 실제 구현에 집중합니다.
...
## 3. Java CompletableFuture 실제 동작 방식

**→ 비동기/동기, 블로킹/논블로킹의 이론적 정의는 `0__비동기_프로그래밍과_Thread_기반_병렬화_완전한_기술_분석.md` 참조**
