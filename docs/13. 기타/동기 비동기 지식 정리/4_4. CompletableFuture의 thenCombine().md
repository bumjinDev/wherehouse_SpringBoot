...
내부적으로 `thenCombine()`은 **중간 조정자 객체(Coordinator)**를 생성한다. 이 조정자가 두 Future의 완료를 모니터링하는 구조다:

```
future1 ─┐
         ├─→ BiRelay(조정자) ──→ combined
future2 ─┘
```

조정자는 두 Future에 각각 콜백을 등록한다:
- `future1`에 등록된 콜백: "내가 완료되면 조정자에게 알림"
- `future2`에 등록된 콜백: "내가 완료되면 조정자에게 알림"

이는 두 Future가 서로를 직접 참조하는 것이 아니라, **제3의 객체가 중재하는 구조**다. 조정자는 내부적으로 완료 카운터를 관리하며, 두 Future 모두로부터 완료 신호를 받았을 때만 다음 단계를 진행한다.

**BiFunction 실행 조건:**

조정자는 두 Future 모두 완료되었을 때만 BiFunction을 실행한다. 먼저 완료된 쪽의 콜백이 조정자를 호출하면 "1개 완료됨" 상태로 전환되고 대기한다. 나중에 완료된 쪽의 콜백이 조정자를 호출하면 "2개 완료됨"을 확인하고 BiFunction을 트리거한다.
...
300ms 시점: future2 완료 → 조정자에 통지 → 카운터 1 증가 → 대기
500ms 시점: future1 완료 → 조정자에 통지 → 카운터 2 도달 → BiFunction 실행
```
...
두 작업이 모두 실패하면 **먼저 감지된 예외**가 `combined`에 전파된다. 이는 스레드 스케줄링과 경합 조건에 따라 달라지므로, 어떤 예외가 전파될지 예측할 수 없다.

```java
CompletableFuture<ProductInfo> future1 = CompletableFuture.supplyAsync(() -> {
    throw new RuntimeException("상품 서비스 장애");
});
...
future1.thenCombine(future2, (info, inv) -> new ProductPage(info, inv))
    .exceptionally(ex -> {
        // 두 예외 중 하나만 전파됨 (어느 것인지 보장 안 됨)
...
각 Future에 개별적으로 예외 처리를 적용하여 부분 실패를 우아하게 처리할 수 있다:

```java
CompletableFuture<ProductInfo> future1 = getProductInfo(productId)
    .exceptionally(ex -> {
        log.error("상품 정보 조회 실패", ex);
        return ProductInfo.empty();  // 기본값 반환
    });

CompletableFuture<Inventory> future2 = getInventoryStatus(productId)
    .exceptionally(ex -> {
        log.error("재고 조회 실패", ex);
        return Inventory.unavailable();  // 기본값 반환
    });

// 두 작업이 모두 기본값으로라도 완료되므로 결합 가능
CompletableFuture<ProductPage> combined = future1.thenCombine(future2,
    (info, inventory) -> new ProductPage(info, inventory)
);
```

이 패턴은 한 서비스의 장애가 전체 페이지 렌더링을 막지 않도록 하는 **회복력 있는 시스템 설계**의 핵심이다.
...
**실행 흐름:**
```
시작: fa, fb, fc 모두 즉시 병렬 실행 시작
완료 시점: fa와 fb 모두 완료되면 Pair 생성
최종 완료: Pair와 fc 모두 완료되면 Result 생성
```

**주의:** 세 작업은 모두 병렬로 실행되지만, **결합 로직의 실행 타이밍**에서 비효율이 발생한다.
...
100ms 시점: fc 완료 → 대기 (아직 Pair가 없음)
500ms 시점: fa, fb 완료 → Pair 생성 → fc와 결합하여 Result 생성
```

fc가 일찍 완료되었지만, Pair가 생성될 때까지 기다려야 한다. 결합 함수의 실행이 계층적으로 연쇄되기 때문이다.
...
500ms 시점: 셋 중 가장 느린 작업 완료 → 즉시 Result 생성
```

**장점:** 세 작업 중 가장 느린 것이 완료되는 즉시 Result 생성. 결합 타이밍이 최적화됨.
**단점:** 타입 안전성 손실, `join()` 호출 필요 (이미 완료된 Future이므로 블로킹은 발생하지 않음)
...
이 방식은 타입 안전성을 어느 정도 회복하면서도 최적의 결합 타이밍을 제공한다.
