# CompletableFuture 메서드 분리의 설계적 우수성
## thenAccept()와 thenAcceptAsync()의 설계 철학 및 책임 경계 분리 원리

---

## 1. 서론: 단일 메서드 통합의 구조적 비효율성
...
### 1.2 CompletableFuture가 달성하려 한 구조적 개선
...
이 분리가 없다면, 비동기 작업의 조합(composition), 재사용(reusability), 그리고 실행 정책의 독립적 제어(independent execution policy control)가 모두 현저히 제한된다. CompletableFuture는 이를 해결하기 위해 각 처리 단계를 독립적인 CompletableFuture 객체로 표현하고, 이들을 조합할 수 있는 연산자들을 제공한다.

이 방식이 비동기 작업 조합 문제를 해결하는 유일한 방법은 아니다. Kotlin Coroutines는 suspend function과 구조화된 동시성으로, Reactive Streams는 Publisher-Subscriber 패턴으로 이 문제에 접근한다. 하지만 CompletableFuture의 메서드 분리 방식은 Java 생태계에서 타입 안정성과 조합성을 동시에 달성하는 가장 효과적인 설계 패턴으로 입증되었다.
...
15개의 외부 API를 병렬로 호출하고 모두 완료되면 결과를 집계하는 시나리오를 고려해보자.

체이닝 방식에서는 각 API 호출이 독립적인 CompletableFuture<ApiResponse>로 존재하므로, CompletableFuture.allOf() 같은 조합 연산자로 이들을 합칠 수 있다. 이때 각 Future는 독립적으로 완료되고, 모든 완료를 기다린 후 집계 로직이 실행된다. 이 과정에서 개별 API 완료 시점은 서로 독립적이며, 전체 파이프라인의 병렬성이 최대한 활용된다.
반면 모든 로직을 supplyAsync() 내부에 넣으면 이런 조합이 극도로 어려워진다. API A를 호출하는 supplyAsync() 안에서 API B의 완료를 기다리려면 futureB.join()을 호출해야 하는데, 이는 API A를 실행하는 워커 스레드를 블로킹시켜버린다. 비동기 프로그래밍은 본질적으로 로직을 독립적으로 분리하여 병렬적으로 실행 가능하도록 설계한 구조인데, 이렇게 하면 워커 스레드의 효율적 활용이라는 핵심 이점이 크게 손상된다. 두 API가 진정으로 병렬 실행되려면, 각각의 완료가 독립적인 Future 객체로 표현되어야 하고, 조합 로직은 별도의 단계(thenCombine, allOf 등)로 분리되어야 한다.

더 복잡한 시나리오에서 이 구조적 열화는 더욱 명확해진다. 10개의 API를 병렬 호출하고 가장 먼저 완료된 3개의 결과만 사용하는 경우, 체이닝 구조에서는 각 API 호출이 독립적인 Future로 존재하므로 List<CompletableFuture<T>>로 관리하고 커스텀 조합 로직을 적용할 수 있다. 하지만 모든 로직이 supplyAsync() 내부에 있다면, 개별 작업의 완료 상태를 외부에서 관찰하거나 제어할 방법이 없다. 각 supplyAsync()는 고립된 실행 블록일 뿐, 다른 작업과의 협력이나 조율이 구조적으로 제한된다. 조합이 완전히 불가능한 것은 아니지만, 동기화 메커니즘과 상태 관리 코드를 수동으로 작성해야 하므로 CompletableFuture가 제공하는 선언적 조합성의 이점이 사라진다.
...
`thenAccept()`는 후속 작업이 경량이고 빠르게 완료될 때 사용한다. 단순 로깅, 간단한 값 변환, 메모리 내 상태 업데이트 등이 이에 해당한다. 이런 경우 스레드 전환으로 인한 컨텍스트 스위칭 비용(시스템과 부하에 따라 수백 나노초에서 수십 마이크로초)과 CPU 캐시 무효화 오버헤드가 작업 자체의 실행 시간보다 클 수 있으므로, 완료 스레드에서 연속 실행하는 것이 효율적이다. 또한 이전 작업과 후속 작업이 논리적으로 하나의 처리 단위일 때, CPU 캐시에 이미 로드된 데이터를 그대로 활용할 수 있어 캐시 미스(cache miss)를 줄일 수 있다.
...
이 실행 결과는 CompletableFuture의 적응적 실행 전략을 명확히 보여준다. Case 1에서는 워커 스레드가 `supplyAsync()` 완료 후 `thenAccept()`까지 연속 실행한다. Case 2에서는 명시적으로 다른 스레드에 작업을 제출한다. Case 3은 Fast Path 최적화를 보여주는데, `thenAccept()` 호출 시점에 원본 Future의 `result` 필드가 이미 null이 아닌 값으로 설정되어 있으므로(정상 완료 또는 예외 완료 상태), completion stack에 노드를 추가하지 않고 호출자 스레드에서 즉시 action을 실행한다.

**Fast Path의 정확한 발동 조건:**
- `thenAccept()` 호출 시점에 `result != null`이면 Fast Path
- 정상 완료 상태: result에 실제 값이 저장된 경우 → action 실행
- 예외 완료 상태: result에 `AltResult` 래퍼로 예외가 저장된 경우 → action 건너뛰고 예외 전파
- `result == null`이면 Slow Path로 진입하여 completion stack에 노드 추가

#### 이 분기 로직은 `uniAcceptStage()` 메서드 내부에서 CAS 연산과 함께 구현되어 있으며, 이는 여러 스레드가 동시에 completion을 등록하려 할 때의 race condition을 안전하게 처리한다.
---
...
Completion들은 내부적으로 Stack 구조로 관리되지만, postComplete()는 이를 역순으로 순회하여 최종적으로 등록된 순서대로(FIFO) 실행한다. 즉, thenAccept() → thenApply() → thenRun() 순서로 체이닝했다면, 실행도 이 순서대로 이루어진다.

각 completion의 실행 중 예외가 발생하면, 해당 completion과 연결된 후속 체인은 예외 상태로 전환되지만, 동일 레벨의 다른 독립적인 completion들은 계속 실행된다. 단, 한 completion에서 처리되지 않은 예외가 발생해도 다른 completion의 실행을 중단시키지는 않는다.

한 completion이 무거운 작업이고 Async 변형이 아니라면, 해당 작업이 완료될 때까지 나머지 completion들은 대기한다. 이는 동일 스레드에서 순차 실행되기 때문이다. 반대로 Async 변형이라면 즉시 Executor에 제출되고, 완료 스레드는 다음 completion 처리로 넘어간다.

이런 동작 방식은 실제 OpenJDK의 CompletableFuture.java 소스코드를 디버깅하며 step-by-step으로 추적해야만 완전히 이해할 수 있다. 특히 postComplete() 메서드 내부의 루프 구조와 각 Completion.tryFire() 메서드의 반환값 처리 로직을 주의 깊게 관찰해야 한다.
...
이는 메모리 가시성(memory visibility)을 보장하기 위한 것으로, volatile 변수에 대한 쓰기 연산 이전의 모든 메모리 연산이 다른 스레드의 volatile 읽기 연산 이후에 올바른 순서로 관찰 가능하도록 보장한다. 이것이 Java Memory Model의 happens-before 관계다.

구체적으로, 한 스레드가 `result` 필드에 완료 값을 쓰면(volatile write), 이후 다른 스레드가 `result` 필드를 읽을 때(volatile read) 다음이 보장된다:
- 쓰기 스레드의 volatile write 이전에 수행된 모든 메모리 연산이 읽기 스레드에게 보이게 된다
- 이는 시간적 "즉시성"을 의미하는 것이 아니라, 인과적 "순서 보장"을 의미한다
- 메모리 배리어(memory barrier)를 통해 CPU 캐시와 메인 메모리 간의 동기화가 강제된다

하지만 volatile의 정확한 의미론—happens-before 관계, 메모리 배리어, 캐시 일관성 프로토콜—을 이해하지 못하면, 왜 이것만으로 충분한지, 언제 추가 동기화가 필요한지 판단할 수 없다. 예를 들어, result 필드의 업데이트는 원자적(atomic)이지만, result 값 자체가 가리키는 객체의 내부 상태는 별도의 동기화 없이는 가시성이 보장되지 않는다.
