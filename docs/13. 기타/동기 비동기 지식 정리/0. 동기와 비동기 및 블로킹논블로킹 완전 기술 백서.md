...
다음 섹션에서 보게 될 JDBC 예제는 이 모든 특성을 실제 코드로 구현한 것이다. 데이터베이스 쿼리는 빠르게 완료되고, 순차 의존성이 있으며, 트랜잭션이 필수적이고, 일반적으로 동시 요청 수를 커넥션 풀 크기로 제한할 수 있다. 따라서 JDBC는 동기-블로킹을 채택한 것이 합리적이다.
...
비동기는 동기의 정반대다. 동기는 호출자가 완료 여부를 직접 확인했지만, 비동기는 **시스템이 완료를 통지**한다.
