...
Worker가 `httpClient.send()`에서 블로킹되면, 해당 스레드는 커널 I/O 대기 상태로 진입한다. 이 Worker의 deque에 다른 작업이 없으므로, 다른 Worker가 훔쳐갈 것도 없다. 

ForkJoinPool은 `ManagedBlocker` 인터페이스를 통해 블로킹을 감지하고 보상 스레드를 추가하는 메커니즘을 제공하지만, 일반적인 I/O 라이브러리(Thread.sleep(), Socket I/O, 대부분의 HTTP 클라이언트)는 이 인터페이스를 구현하지 않는다. 따라서 실제 환경에서는 블로킹이 감지되지 않아 새 Worker가 생성되지 않는다.

**불일치 3: 스레드 수 고정의 문제**

ForkJoinPool의 기본 parallelism은 CPU 코어 수 (예: 4개)다. 일반적인 블로킹 I/O는 ForkJoinPool의 블로킹 감지 메커니즘을 활용하지 않으므로, 4개 Worker가 모두 블로킹되면 새 작업을 처리할 스레드가 없다. CPU는 유휴 상태지만 작업은 큐에서 대기한다.
