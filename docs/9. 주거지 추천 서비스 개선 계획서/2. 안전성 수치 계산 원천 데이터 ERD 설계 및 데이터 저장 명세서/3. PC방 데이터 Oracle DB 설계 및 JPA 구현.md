# PC방 데이터 Oracle DB 설계 및 JPA 구현

## 1. Oracle 테이블 DDL

```sql
CREATE TABLE PC_BANGS (
    ...
    KARAOKE_ROOMS           NUMBER(6),
    YOUTH_ROOMS             NUMBER(6),
...
CREATE SEQUENCE SEQ_PC_BANGS START WITH 1 INCREMENT BY 1;
```
...
@AllArgsConstructor
@Builder
public class PcBangs {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "pc_bang_seq")
    @SequenceGenerator(name = "pc_bang_seq", sequenceName = "SEQ_PC_BANGS", allocationSize = 1)
    @Column(name = "ID")
    private Long id;
...
import com.wherehouse.safety.entity.PcBangs;
...
import com.wherehouse.safety.entity.PcBangs;
import com.wherehouse.safety.repository.PcBangsRepository;
...
@Slf4j
public class PcBangsDataLoader implements CommandLineRunner {

    private final PcBangsRepository pcBangsRepository;
    private final ResourceLoader resourceLoader;

    @Value("${app.csv.pc-bangs-data-path}")
    private String csvFilePath;

    @Override
    @Transactional
    public void run(String... args) {
        if (pcBangsRepository.count() > 0) {
            log.info("PC방 데이터 이미 존재. 로딩 스킵");
            return;
        }
        ...
        try {
            ...
            try (CSVReader reader = new CSVReader(new InputStreamReader(resource.getInputStream(), StandardCharsets.UTF_8))) {
                ...
                log.info("PC방 데이터 로딩 시작 - 총 {} 건", totalRows);
                ...
                for (int i = 1; i < csvData.size(); i++) {
                    ...
                    if (row.length >= 50) {
                        try {
                            PcBangs pcBang = PcBangs.builder()
                        ...
                        errorCount++;
                        log.warn("{}번째 행 컬럼 부족 ({}개 < 50개)", i, row.length);
                    }
                }

                log.info("PC방 데이터 로딩 완료 - 성공: {}건, 실패: {}건, 전체: {}건",
...
파일 위치: `src/main/resources/data/PC방_데이터.csv`