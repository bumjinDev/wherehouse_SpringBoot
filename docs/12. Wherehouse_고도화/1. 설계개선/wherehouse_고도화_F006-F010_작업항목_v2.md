# Wherehouse 고도화 — F006~F010 구현 작업 항목
...
## 3. F009 — 검색 인덱스 동기화 쓰기 시퀀스의 Write-Read 경합 해소

### 핵심 의사결정 질문
> "syncRedisAfterUpdate/StatusChange의 Hash+ZSet 쓰기 시퀀스가 원자적이지 않아, Reader가 중간 상태(ZSet Score는 구값, Hash는 신값)를 관찰하여 잘못된 추천 결과를 산출하는 문제를 어떻게 해소하는가?"

### 3-1. 현재 문제 분석 — 경로별 영향 차이

syncRedisAfterCreate/Update/StatusChange는 매물 데이터를 Redis에 반영할 때 다음 3개 커맨드를 순차 실행한다:

```java
// ① Hash 갱신
redisHandler.redisTemplate.opsForHash().putAll(hashKey, hashFields);
// ② 가격 인덱스 갱신
redisHandler.redisTemplate.opsForZSet().add(priceIndexKey, propertyId, deposit);
// ③ 평수 인덱스 갱신
redisHandler.redisTemplate.opsForZSet().add(areaIndexKey, propertyId, areaInPyeong);
```

각 커맨드는 개별적으로 원자적이지만, ①~③ 사이에 Reader(CharterRecommendationService)의 Redis 커맨드가 끼어들 수 있다.

이 문제의 영향은 Write 경로(CREATE/UPDATE/STATUS CHANGE)에 따라 다르다.

**CREATE 경로 — 문제 없음 (교집합 필터에 의한 자연 보호)**

신규 매물은 ①②③ 실행 전에 어떤 인덱스에도 존재하지 않는다. Reader의 `findValidCharterPropertiesInDistrict()`는 가격 인덱스와 평수 인덱스의 교집합(`retainAll`)으로 후보를 선별하므로, 두 인덱스에 모두 등록되기 전에는 해당 매물이 교집합에 포함되지 않는다.

```
① 직후: 가격 인덱스에 A 없음, 평수 인덱스에 A 없음 → 교집합 미포함
② 직후: 가격 인덱스에 A 있음, 평수 인덱스에 A 없음 → 교집합 미포함
③ 직후: 양쪽 모두 A 있음 → 교집합 포함 → Hash(①에서 완료)로 정상 점수 계산
```

Reader가 매물 A를 "볼 수 있는 시점"(③ 완료)에서 Hash 데이터(①)는 이미 완료 상태이므로, 잘못된 점수가 산출되지 않는다. 유일한 결과는 "아직 안 보인다"(부재)이지 "잘못된 데이터로 계산된다"(오류)가 아니다.

**UPDATE 경로 — 문제 발생 (ZSet Score 구값 vs Hash 신값 불일치)**

기존 매물이 이미 양쪽 인덱스에 존재하는 상태에서 가격을 변경하는 경우.

```
전제: 매물 A, deposit=50000 → 80000으로 변경. Reader 예산 범위: 40000~60000.

① HMSET property:charter:A {deposit:80000}    ← Hash 갱신 완료
   ↓ 이 시점에 Reader 조회:
     ZRANGEBYSCORE idx:charterPrice:강남구 40000 60000
     → A의 ZSet Score = 50000 (구값, 미갱신) → 범위 내 → A 포함 통과
     HGETALL property:charter:A
     → deposit = 80000 (신값, ①에서 갱신됨)
     → 예산 범위(40000~60000) 밖 매물이 추천 결과에 포함됨

② ZADD idx:charterPrice:강남구 80000 A        ← 가격 인덱스 갱신 완료
   이후 Reader 조회: Score=80000 → 범위 밖 → A 탈락 (정상)
```

ZSet Score(필터 기준)는 구값이어서 범위 필터를 통과하지만, Hash(실제 계산 데이터)는 이미 신값으로 갱신되어 있다. Reader는 사용자 예산을 초과하는 매물을 추천 결과에 포함시키고, 80000 기준으로 점수를 계산한다.

**STATUS CHANGE 경로 — 문제 발생 (Hash=INACTIVE, ZSet에 잔존)**

매물을 INACTIVE로 전환하는 경우.

```
전제: 매물 A, ACTIVE 상태. 소유자가 INACTIVE로 전환.

① HSET property:charter:A status INACTIVE    ← Hash 상태 갱신 완료
   ↓ 이 시점에 Reader 조회:
     양쪽 ZSet 인덱스에 A 존재 → 교집합 포함 → 통과
     HGETALL → status=INACTIVE
     → 비활성 매물이 추천 결과에 포함됨

② ZREM idx:charterPrice:강남구 A              ← 가격 인덱스 삭제 완료
   이후 Reader 조회: 가격 인덱스에 A 없음 → 교집합 탈락 (정상)
   (평수 인덱스에 A가 잔존하더라도 교집합에서 탈락하므로 무해)

③ ZREM idx:area:강남구:전세 A                 ← 평수 인덱스 삭제 완료
```

② 이후부터는 CREATE 경로와 동일한 교집합 보호가 작동한다. 위험 윈도우는 ①~② 사이의 단일 구간이다.

> 테스트 완료 — `F009_WriteRead경합_테스트결과.md` 참조

### 3-2. 대안 비교 분석

문제의 위치가 JVM 내부가 아닌 Redis 서버 측이라는 점이 대안 선정의 전제이다. Java 코드에서 아무리 스레드 동기화를 적용해도, Redis 서버에 도착하는 커맨드의 인터리빙은 막을 수 없다. Writer와 Reader는 서로 다른 HTTP 요청에서 온 서로 다른 서비스 메서드를 실행하는 서로 다른 스레드이다.

**대안 A: synchronized (JVM 레벨 락)**
- 동작: Writer의 syncRedisAfterUpdate/StatusChange 메서드에 synchronized 적용.
- 한계: Writer 스레드 간 코드 블록 동시 진입은 막지만, Reader가 별도 HTTP 요청에서 별도 서비스 메서드를 통해 전송하는 Redis 커맨드는 이 모니터와 무관하게 Redis 서버 큐에 도착한다. Redis 서버 측 인터리빙을 차단하지 못한다.
- 판정: 탈락 — 문제 발생 위치(Redis 서버)와 보호 범위(JVM 내부)가 불일치.

**대안 B: Redis Pipeline (executePipelined)**
- 동작: 3개 커맨드를 하나의 네트워크 왕복에 묶어 전송.
- 한계: Pipeline은 네트워크 왕복 횟수를 줄이는 최적화이지, Redis 서버 측에서 커맨드 사이에 다른 클라이언트의 커맨드가 끼어드는 것을 막지 않는다. Pipeline으로 전송된 3개 커맨드 사이에도 Reader의 커맨드가 끼어들 수 있다.
- 판정: 탈락 — 네트워크 최적화일 뿐 원자성 미보장.

**대안 C: MULTI/EXEC (Redis 트랜잭션)**
- 동작: Writer가 MULTI를 보낸 후 3개 커맨드를 전송하면 Redis 서버가 즉시 실행하지 않고 내부 큐에 적재. EXEC 전송 시 적재된 커맨드를 연속 실행하며, 이 구간에 다른 클라이언트의 커맨드가 끼어들 수 없다.
- 장점: Reader가 관찰 가능한 상태가 "변경 전 전체" 또는 "변경 후 전체"로 한정됨. 중간 상태 관찰 원천 차단.
- 구현: Spring Data Redis의 SessionCallback 패턴.
- 판정: 선택 — 문제 위치(Redis 서버)에서 직접 원자성을 보장하는 유일한 대안.

| 기준 | A: synchronized | B: Pipeline | C: MULTI/EXEC |
|------|----------------|-------------|---------------|
| 동작 위치 | JVM 내부 | 네트워크 계층 | Redis 서버 내부 |
| Reader 인터리빙 차단 | 불가 | 불가 | 가능 |
| 중간 상태 관찰 방지 | 불가 | 불가 | 가능 |
| 구현 복잡도 | 낮음 | 낮음 | 중간 (SessionCallback) |
| 판정 | 탈락 | 탈락 | **선택** |
...
선택 근거: 문제의 위치가 Redis 서버 측 커맨드 인터리빙이므로, Redis 서버 자체가 원자성을 보장하는 메커니즘만 유효하다. synchronized와 Pipeline은 각각 JVM과 네트워크 계층에서만 동작하여 이 문제를 해결하지 못한다.

**구현 작업:**

1. `CharterPropertyWriteService`, `MonthlyPropertyWriteService`의 syncRedisAfterUpdate(), syncRedisAfterStatusChange() 내부에서 Hash+ZSet 쓰기 시퀀스를 SessionCallback 기반 MULTI/EXEC로 전환.
...
redisHandler.redisTemplate.execute(new SessionCallback<List<Object>>() {
    ...
    public List<Object> execute(RedisOperations operations) {
        ...
        operations.opsForZSet().add(priceIndexKey, propertyId, deposit);
        operations.opsForZSet().add(areaIndexKey, propertyId, areaInPyeong);
        return operations.exec();
    }
});
```

2. syncRedisAfterStatusChange()의 INACTIVE 전환 시 Hash 갱신 + ZSet 삭제(ZREM)도 동일하게 MULTI/EXEC로 묶음.

3. syncRedisAfterCreate()는 교집합 필터에 의해 자연 보호되므로 MULTI/EXEC 적용 대상에서 제외. 단, F008의 부분 실패 보상 전략 구현 시 일관성을 위해 MULTI/EXEC를 적용할 수 있으며, 이 경우 부분 실패 방지와 원자적 가시성을 동시에 달성한다.

**CREATE 경로의 MULTI/EXEC 적용 여부는 F008 구현 시점에 확정한다.**

### 3-4. 검증

- 테스트 1: UPDATE 정상 흐름 — 가격 변경 후 Hash deposit과 ZSet Score가 일치하는지 확인.
- 테스트 2: UPDATE Write-Read 경합 재현 — Writer 스레드에서 Hash 갱신과 ZSet 갱신 사이에 인위적 지연 삽입. Reader 스레드가 동시 조회 시 구값 전체 또는 신값 전체만 관찰하는지 확인. MULTI/EXEC 적용 전/후 비교.
- 테스트 3: STATUS CHANGE Write-Read 경합 재현 — Hash status=INACTIVE 갱신과 ZSet ZREM 사이에 인위적 지연 삽입. Reader 스레드가 비활성 매물을 추천 결과에 포함시키는지 확인. MULTI/EXEC 적용 전/후 비교.
...
문서 구조:
1. **문제 정의**: Hash+ZSet 쓰기 시퀀스의 Write-Read 경합. CREATE/UPDATE/STATUS CHANGE 경로별 영향 차이 분석.
2. **대안 3개 비교표**: synchronized·Pipeline·MULTI/EXEC. 문제 위치(Redis 서버)와 각 대안의 보호 범위(JVM/네트워크/Redis 서버) 불일치에 의한 탈락 근거.
3. **선택 근거**: MULTI/EXEC가 유일하게 Redis 서버 측에서 원자성을 보장.
4. **검증 결과**: UPDATE·STATUS CHANGE Write-Read 경합 재현 테스트 전/후 비교.

---

## 4. F010 — 정규화 경계값(bounds) 원자적 갱신

### 핵심 의사결정 질문
> "BoundsUpdater.tryExtend()의 HGET→비교→HSET 비원자적 시퀀스가 동시 매물 등록 환경에서 bounds 정합성을 어떻게 훼손하는가? 이를 원자적으로 보장하기 위해 어떤 전략이 적합한가?"

### 4-0. 전제 재검증 — Sorted Set Score의 실제 용도
...
### 4-6. CPU 바운드 구간 이관 — 추천 조회 경로 병렬화

초기 계획에서 F009 범위로 포함했던 "CPU 바운드 재계산의 ForkJoinPool 병렬화"는 Write 경로에 재계산 작업이 존재하지 않으므로 F010에서 분리한다.
...
## 5. F007 — 배치-사용자 데이터 충돌 처리 (임계값 기반 머지)
...
2. F006 (중복 감지)
   ...
   2-5. 설계 결정 문서 작성
   ↓
3. F009 (검색 인덱스 동기화 Write-Read 경합 해소)
   3-1. 경로별 영향 분석 (CREATE=자연보호 / UPDATE·STATUS CHANGE=불일치 발생)
   3-2. 대안 3개 비교 (synchronized / Pipeline / MULTI/EXEC)
   3-3. 선택 + 구현 (MULTI/EXEC — 문제 위치가 Redis 서버 측)
   3-4. 검증 (UPDATE·STATUS CHANGE Write-Read 경합 재현 전/후 비교)
   3-5. 설계 결정 문서 작성
   ↓
4. F010 (bounds 원자적 갱신)
   4-0. 전제 재검증 — Sorted Set Score 실제 용도 확인 → ScoreRecalc 경로 폐기
   4-1. BoundsUpdater 비원자성 문제 2가지 시나리오 문서화 + MULTI/EXEC 단독 탈락 근거
   4-2. 대안 비교 (WATCH+MULTI/EXEC / 현행 유지)
   4-3. 선택 + 구현
   4-4. 검증 (동시 등록 bounds 정합성 + Write-Read 경합)
   4-5. 설계 결정 문서 작성
   4-6. CPU 바운드 구간 → 추천 조회 경로 병렬화 별도 항목으로 이관
   ↓
5. F007 (배치-사용자 머지)
   5-1. 기존 배치 코드 파악
   5-2. 임계값 산출 기준 설계 (실데이터 분석)
   5-3. 머지 로직 구현
   5-4. 동시 쓰기 대응 (설계 판단 문서화)
   5-5. 검증
   5-6. 설계 결정 문서 작성
```
...
예시 (F009):
- 비즈니스 맥락: "매물 정보가 수정되면 캐시의 상세 데이터와 검색 인덱스가 함께 갱신되는데, 갱신 도중에 다른 사용자가 추천 검색을 하면 '수정 전 인덱스 + 수정 후 상세 데이터'가 섞여 예산 범위를 벗어난 매물이 추천되거나, 비활성 처리한 매물이 노출될 수 있다"
- 설계 선택지: "여러 캐시 갱신 명령을 어떻게 하나의 단위로 묶어서 중간 상태 노출을 막을 것인가"
- 선택 근거: "이 문제는 애플리케이션(Java) 내부가 아니라 캐시 서버(Redis)에서 발생하므로, Java 레벨 락(synchronized)이나 네트워크 최적화(Pipeline)로는 해결 불가. Redis 서버 자체가 명령 묶음의 원자적 실행을 보장하는 MULTI/EXEC 선택"
- 검증 결과: "갱신 중간에 조회 요청을 동시 실행하는 테스트로, 적용 전 불일치 발생 / 적용 후 변경 전 또는 변경 후 전체 상태만 관찰됨을 확인"
...
- F009와의 차이: "F009는 단순 쓰기 묶음이어서 MULTI/EXEC로 해결되지만, 이 문제는 읽은 값에 따라 쓸지 말지를 판단하는 구조여서 MULTI/EXEC 단독으로는 해결 불가"
- 선택 근거: (대안 선택 후 기록)
