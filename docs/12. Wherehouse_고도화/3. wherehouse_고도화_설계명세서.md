...
v2.0(2146줄)이 기획서·요구사항 명세서와의 외부 중복 및 섹션 간 내부 중복으로 설계 고유 정보의 식별이 어려웠던 문제를 해소한다. 외부 중복(기능 개요·CQRS 역할 분담·비기능 요구·기존 시스템 한계)은 원 문서 참조로 대체하고, 내부 중복(RDB·Redis 순서 원칙·스키마 공존 메커니즘·권한 검증 3단계·에러 코드 상세·수정되지 않는 컴포넌트 목록)은 단일 진원지로 통합한다. 설계 고유 결정(API 계약·데이터 모델·상태 관리·처리 단계 차이점)에 집중한다.
...
sequenceDiagram
    ...
    Client->>Filter: POST /api/v1/properties<br/>인증 토큰 포함
...
|------|-------------|----------|
| F001 매물 등록 | POST | `/api/v1/properties` |
| F002 매물 수정 | PATCH | `/api/v1/properties/{propertyId}` |
| F003 매물 상태 변경 | PATCH | `/api/v1/properties/{propertyId}/status` |
| F004 매물 목록 조회 | GET | `/api/v1/properties` |
| F004 매물 상세 조회 | GET | `/api/v1/properties/{propertyId}` |

F002·F003가 모두 PATCH를 사용하는 근거는 두 API가 매물 리소스의 일부 필드만 수정하는 부분 갱신 연산이기 때문이다. PUT은 리소스 전체 대체 시맨틱을 가지며 이 경우 불변 속성까지 요청 본문에 포함해야 하는 불일치가 발생한다. F003의 `/status` 서브 경로 분리 근거는 상태 변경 전용 API임을 API 계약 레벨에서 명확히 드러내어 F002의 전체 가변 필드 수정과 의미를 구분하기 위함이다.
...
| API | 인증 요구 | 권한 검증 |
|-----|----------|----------|
| F001 POST /api/v1/properties | 필수 (Protected) | 인증만 필요, 별도 권한 없음 |
| F002 PATCH /api/v1/properties/{id} | 필수 (Protected) | 매물 등록자 본인 검증 (9.2.3) |
| F003 PATCH /api/v1/properties/{id}/status | 필수 (Protected) | 매물 등록자 본인 검증 (9.2.3 공유) |
...
- **권한 관리**: F002·F003의 등록자 본인 검증은 매물 쓰기 서비스 계층의 내부 헬퍼에서 3단계(매물 존재 → 등록자 존재 → 본인 일치)로 수행된다. 단일 진원지는 섹션 9.2.3이며 F003도 동일 로직을 공유한다(섹션 9.3.3).
...
| 컴포넌트 | 패키지 | 역할 |
|---------|--------|------|
| `PropertyWriteController` | `com.wherehouse.property.controller` | F001·F002·F003 쓰기 API 단일 진입점. 인증 컨텍스트에서 사용자 식별자 추출 |
| `PropertyQueryController` | `com.wherehouse.property.controller` | F004 매물 목록·상세 조회 API 진입점 |
| `PropertyBoardViewController` | `com.wherehouse.property.controller` | 매물 등록·수정·목록 JSP 뷰 렌더링 |
| `GlobalExceptionHandlerProperty` | `com.wherehouse.property.controller` | 매물 도메인 예외의 HTTP 응답 매핑 |

**서비스 계층 (신규)**

| 컴포넌트 | 패키지 | 역할 |
|---------|--------|------|
| `PropertyWriteService` | `com.wherehouse.property.service` | F001~F003 CUD의 원자성 관리, 등록자 본인 검증, 식별자 생성 호출, 후속 인터페이스 호출 |
| `PropertyQueryService` | `com.wherehouse.property.service` | F004 매물 전체 조회. RDB 직접 참조, 복합 필터·페이지네이션·정렬 |

**저장소 접근 계층 (신규)**

| 컴포넌트 | 패키지 | 역할 |
|---------|--------|------|
| `PropertyCharterRepository` | `com.wherehouse.property.repository` | 전세 매물 테이블 접근, 동적 쿼리 지원 |
| `PropertyMonthlyRepository` | `com.wherehouse.property.repository` | 월세 매물 테이블 접근, 동적 쿼리 지원 |

**도메인 모델 계층 (신규)**

| 컴포넌트 | 패키지 | 역할 |
|---------|--------|------|
| `PropertyCharterEntity` | `com.wherehouse.property.entity` | `PROPERTIES_CHARTER` 매핑 |
| `PropertyMonthlyEntity` | `com.wherehouse.property.entity` | `PROPERTIES_MONTHLY` 매핑. 월세 전용 컬럼 보유 |
| `PropertyStatus` | `com.wherehouse.property.entity` | 매물 상태 Enum (`ACTIVE`·`COMPLETED`·`DELETED`) |
| `DataSource` | `com.wherehouse.property.entity` | 데이터 출처 Enum (`BATCH`·`USER`·`MERGED`) |
...
**후속 고도화 인터페이스 (신규 — F006·F008·F009 경계)**

| 컴포넌트 | 패키지 | 역할 |
|---------|--------|------|
| `DuplicateChecker` | `com.wherehouse.property.integration` | F006 중복 감지 인터페이스 (경계만 정의, 구현은 F006 후속 설계) |
| `IndexSyncService` | `com.wherehouse.property.integration` | F008 인덱스 동기화 인터페이스 (경계만 정의, 구현은 F008 후속 설계) |
| `ScoreRecalcService` | `com.wherehouse.property.integration` | F009 점수 재계산 인터페이스 (경계만 정의, 구현은 F009 후속 설계) |

**예외 계층 (신규)**

| 컴포넌트 | 패키지 | 매핑 HTTP 상태 |
|---------|--------|---------------|
| `PropertyNotFoundException` | `com.wherehouse.property.exception` | 404 |
| `UnauthorizedPropertyAccessException` | `com.wherehouse.property.exception` | 403 |
| `InvalidStateForUpdateException` | `com.wherehouse.property.exception` | 409 (종료 상태 수정) |
| `InvalidStatusTransitionException` | `com.wherehouse.property.exception` | 400 (상태 전이 위반) |
| `DuplicatePropertyException` | `com.wherehouse.property.exception` | 409 (중복 등록, F006 경계에서 전달) |

**DTO 계층 (신규)**

F001~F003 요청·응답 각 2개 + F004 목록 요청·응답·Summary·Detail 4개 = 총 10개. 전체 목록은 섹션 11.4 참조.
...
|---------|------|---------------|-----------|
| `ACTIVE` | 거래 진행 중 매물. 등록 직후의 기본 상태 | 포함 | 등록자 본인 수정·전이 가능 |
...
| F002 가변 속성 수정 | 등록자 본인 가능 | 불가 (E4106) | 불가 (404 또는 E4106) |
...
| 필드 | 타입 | 필수 | 검증 규칙 |
|------|------|------|---------|
| `leaseType` | String | Y | `CHARTER` 또는 `MONTHLY` |
| `aptNm` | String | Y | 1~100자 |
| `sggCd` | String | Y | 서울시 25개 자치구 코드 중 하나 |
| `umdNm` | String | Y | 1~100자 |
| `jibun` | String | Y | 1~50자 |
| `floor` | Integer | Y | -10 이상 100 이하 (음수는 지하층) |
| `excluUseAr` | Number | Y | 양수, 소수점 4자리까지 |
| `buildYear` | Integer | N | 1900~현재 연도 |
| `dealDate` | String | N | ISO 8601 날짜 (`YYYY-MM-DD`) |

**Request Body — 임대 유형별 필드**

| 필드 | 타입 | 필수 조건 | 설명 |
|------|------|----------|------|
| `deposit` | Integer | 전세·월세 모두 필수 | 전세금 또는 월세 보증금 (만원) |
| `monthlyRent` | Integer | 월세일 때만 필수 | 월세금 (만원). 전세 요청 포함 시 E4001 |
...
**Request Body** — PATCH 시맨틱, 변경 필드만 포함

| 필드 | 타입 | 선택성 | 제약 |
|------|------|------|------|
| `deposit` | Integer | 선택 | 양수 |
| `monthlyRent` | Integer | 선택 | 양수, 월세 매물만. 전세 매물에 포함 시 E4001 |
| `buildYear` | Integer | 선택 | 1900~현재 연도 |
| `dealDate` | String | 선택 | ISO 8601 날짜 |

**수정 불가 필드**: 불변 속성 5개(`sggCd`·`jibun`·`aptNm`·`floor`·`excluUseAr`)와 시스템 관리 필드(`propertyId`·`dataSource`·`status`·`registeredUserId`·`registeredAt`·`modifiedAt`). 요청 본문에 포함 시 E4001.
...
발생 가능 코드: **E4001, E4101, E4003, E4201, E4106**. 상세는 섹션 7.8. E4003은 등록자 본인 아님 또는 배치 매물 수정 시도(섹션 9.2.3 3단계 검증).
...
|------|------|
| HTTP Method | PATCH |
| Endpoint URL | `/api/v1/properties/{propertyId}/status` |
| 인증 여부 | 필수 |
| 권한 검증 | 등록자 본인 (섹션 9.2.3 공유) |
| 상태 전이 규칙 | 섹션 6.2 |
...
`PropertyDetailDto`는 `PropertySummaryDto`(7.5.2)의 모든 필드에 다음을 추가한다.
...
| `REGISTERED_USER_ID` | `VARCHAR2(50)` | — | YES | 사용자 매물의 등록자 식별자. 배치는 NULL |
...
F001은 인증된 사용자가 매물 정보를 입력하여 신규 매물을 등록하는 기능이다. API 엔드포인트는 `POST /api/v1/properties`(섹션 7.2).
...
| 검증 항목 | 대상 필드 | 검증 규칙 |
|---------|---------|---------|
| 필수 필드 존재 | 섹션 7.2.1 공통 필수 필드 전체 | 누락 시 E4001 |
| 값 범위 | `floor`·`excluUseAr`·`buildYear`·`deposit`·`monthlyRent` | 섹션 7.2.1 |
| Enum 값 범위 | `leaseType` | 섹션 8.3 |
| 패턴 일치 | `dealDate` | ISO 8601 날짜 형식 |
| 임대 유형별 조건부 필수성 | `monthlyRent` | 월세 요청 시 필수, 전세 요청 포함 시 E4001 |
| 지역 코드 허용 값 | `sggCd` | 서울시 25개 자치구 코드만 |
...
F002는 매물 등록자가 본인 등록 매물의 가변 속성을 수정하는 기능이다. API 엔드포인트는 `PATCH /api/v1/properties/{propertyId}`(섹션 7.3). 등록자 본인만 수정 가능하도록 3단계 권한 검증이 필수다.
...
| R-F002-02 | 요청 DTO 바인딩 및 1차 유효성 검증 | 쓰기 API 진입점 | 9.2.2의 1차 계층. 불변 속성 수정 시도 시 E4001 |
| R-F002-03 | 매물 조회 및 권한 검증 3단계 | 쓰기 서비스 계층 | 9.2.3 로직. 단계별 실패 시 E4201 또는 E4003 |
| R-F002-04 | 종료 상태 수정 차단 | 쓰기 서비스 계층 | 현재 상태가 `COMPLETED`이면 E4106. `ACTIVE`만 수정 허용 |
| R-F002-05 | 매물 레코드 갱신 | 쓰기 서비스 계층 → 저장소 접근 계층 | 요청 본문의 가변 속성만 갱신. `MODIFIED_AT=현재 시각` 동시 갱신 |
| R-F002-06 | RDB 변경 확정 | 쓰기 서비스 계층 | 원자성 경계 종료 |
| R-F002-07 | 인덱스 동기화 호출 | 쓰기 서비스 계층 → `IndexSyncService` | 변경된 속성에 해당하는 인덱스만 갱신 |
| R-F002-08 | 점수 재계산 연쇄 호출 | `IndexSyncService` → `ScoreRecalcService` | `bounds` 변경 감지 시에만 |
| R-F002-09 | 응답 DTO 반환 | → 클라이언트 | 200 OK (섹션 7.3.2) |
...
| 패턴 일치 | `dealDate` | ISO 8601 날짜 형식 |
| 불변 속성 수정 시도 차단 | `sggCd`·`jibun`·`aptNm`·`floor`·`excluUseAr` | 요청 본문 포함 시 E4001 |
| 시스템 관리 필드 수정 시도 차단 | `propertyId`·`dataSource`·`status`·`registeredUserId`·`registeredAt`·`modifiedAt` | 요청 본문 포함 시 E4001 |
| 임대 유형별 필드 정합성 | `monthlyRent` | 전세 매물에 포함 시 E4001 |

불변 속성 수정 시도 차단이 1차 계층에 배치되는 근거는 요청 값 자체의 형식 적합성 판단이며 DB 조회가 불필요하기 때문이다.

**2차 계층: 비즈니스 규칙 검증** (R-F002-03·R-F002-04)

| 검증 항목 | 검증 규칙 | 실패 응답 |
|---------|---------|---------|
| 매물 존재 및 상태 | `ACTIVE`·`COMPLETED` 상태 레코드만 유효 | E4201 |
| 권한 3단계 (9.2.3) | 요청자가 매물 등록자 본인 | E4003 |
| 종료 상태 수정 차단 (R-F002-04) | `ACTIVE`만 수정 허용 | E4106 |

#### 9.2.3 권한 검증 3단계 로직 (F002·F003 공통 단일 진원지)

F002·F003는 매물 등록자 본인만 수정·상태 변경할 수 있어야 한다. 매물 쓰기 서비스 계층의 내부 헬퍼가 다음 3단계를 순차 수행하며 각 단계의 실패는 구분된 에러 코드로 매핑된다. **F003도 동일 로직을 공유한다**(9.3.3).
...
F003은 매물 등록자가 본인 매물의 상태를 `ACTIVE`에서 `COMPLETED` 또는 `DELETED`로 전이시키는 기능이다. API 엔드포인트는 `PATCH /api/v1/properties/{propertyId}/status`(섹션 7.4). 등록자 본인 권한 검증과 상태 전이 허용성 검증이 모두 필수다. 상태 전이 규칙 자체의 정의는 섹션 6.2.
...
| `PROPERTIES_CHARTER` | `REGISTERED_USER_ID` | `VARCHAR2(50)` | — | YES | 등록자 식별 |
...
| `PROPERTIES_MONTHLY` | `REGISTERED_USER_ID` | `VARCHAR2(50)` | — | YES | 동일 |
...
섹션 5.2의 신규 컴포넌트 목록을 파일 단위로 집계한 형태이다. 모든 신규 파일은 `com.wherehouse.property` 패키지 하위에 배치된다.

**API 계층 (4개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/controller/PropertyWriteController.java` | F001·F002·F003 쓰기 API 진입점 |
| `com/wherehouse/property/controller/PropertyQueryController.java` | F004 조회 API 진입점 |
| `com/wherehouse/property/controller/PropertyBoardViewController.java` | 매물 등록·수정·목록 JSP 뷰 렌더링 |
| `com/wherehouse/property/controller/GlobalExceptionHandlerProperty.java` | 매물 도메인 예외 HTTP 응답 매핑 |

**서비스 계층 (2개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/service/PropertyWriteService.java` | F001~F003 CUD 원자성 관리, 후속 인터페이스 호출 |
| `com/wherehouse/property/service/PropertyQueryService.java` | F004 매물 전체 조회, RDB 직접 참조 |

**저장소 접근 계층 (2개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/repository/PropertyCharterRepository.java` | 전세 매물 테이블 접근 |
| `com/wherehouse/property/repository/PropertyMonthlyRepository.java` | 월세 매물 테이블 접근 |

**도메인 모델 계층 (4개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/entity/PropertyCharterEntity.java` | `PROPERTIES_CHARTER` 매핑 |
| `com/wherehouse/property/entity/PropertyMonthlyEntity.java` | `PROPERTIES_MONTHLY` 매핑 |
| `com/wherehouse/property/entity/PropertyStatus.java` | 매물 상태 Enum |
| `com/wherehouse/property/entity/DataSource.java` | 데이터 출처 Enum |

**후속 고도화 인터페이스 계층 (3개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/integration/DuplicateChecker.java` | F006 인터페이스 |
| `com/wherehouse/property/integration/IndexSyncService.java` | F008 인터페이스 |
| `com/wherehouse/property/integration/ScoreRecalcService.java` | F009 인터페이스 |

**예외 계층 (5개)**

| 파일 경로 | 매핑 HTTP 상태 |
|----------|---------------|
| `com/wherehouse/property/exception/PropertyNotFoundException.java` | 404 |
| `com/wherehouse/property/exception/UnauthorizedPropertyAccessException.java` | 403 |
| `com/wherehouse/property/exception/InvalidStateForUpdateException.java` | 409 (종료 상태 수정) |
| `com/wherehouse/property/exception/InvalidStatusTransitionException.java` | 400 (상태 전이 위반) |
| `com/wherehouse/property/exception/DuplicatePropertyException.java` | 409 (F006 경계에서 전달) |

**DTO 계층 (10개)**

| 파일 경로 | 역할 |
|----------|------|
| `com/wherehouse/property/dto/PropertyCreateRequestDto.java` | F001 요청 |
| `com/wherehouse/property/dto/PropertyCreateResponseDto.java` | F001 응답 |
| `com/wherehouse/property/dto/PropertyUpdateRequestDto.java` | F002 요청 |
| `com/wherehouse/property/dto/PropertyUpdateResponseDto.java` | F002 응답 |
| `com/wherehouse/property/dto/PropertyStatusUpdateRequestDto.java` | F003 요청 |
| `com/wherehouse/property/dto/PropertyStatusUpdateResponseDto.java` | F003 응답 |
| `com/wherehouse/property/dto/PropertyListRequestDto.java` | F004 목록 Query Parameter 바인딩 |
| `com/wherehouse/property/dto/PropertyListResponseDto.java` | F004 목록 응답 (페이지네이션) |
| `com/wherehouse/property/dto/PropertySummaryDto.java` | F004 목록의 개별 매물 요약 |
| `com/wherehouse/property/dto/PropertyDetailDto.java` | F004 상세 응답 |

**JSP 뷰 파일**

기존 리뷰 게시판과 동일 구조로 매물 등록·수정·목록 뷰를 신규 추가한다. 구체 파일명은 구현 단계에서 확정.

**총 신규 파일 수**: 자바 30개(API 4 + 서비스 2 + 저장소 2 + 도메인 4 + 인터페이스 3 + 예외 5 + DTO 10) + JSP 뷰 수 개. 구현 단계에서 실제 수 미세 변동 가능.
...
|------|------|---------|
| v2.0 | 2026-04-21 | 초안 |
| v2.1 | 2026-04-21 | 외부·내부 중복 제거, 단일 진원지 원칙 적용, 설계 고유 정보로 집중 |
