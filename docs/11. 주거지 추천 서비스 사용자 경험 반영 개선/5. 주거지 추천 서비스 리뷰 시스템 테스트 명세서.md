...
**Endpoint**: `GET {{base_url}}/api/v1/reviews`
...
```
GET {{base_url}}/api/v1/reviews?property_id={{test_property_id}}&page=1&size=10&sort=latest
```

**케이스 B: 전체 리뷰 조회 (게시판용)**
```
GET {{base_url}}/api/v1/reviews?page=1&size=10&sort=latest
```

**케이스 C: 별점 정렬**
```
GET {{base_url}}/api/v1/reviews?property_id=5d41402abc4b2a76b9719d911017c592&page=1&size=10&sort=rating_desc
```
- `sort` 옵션: `latest`, `rating_desc`, `rating_asc`
- Query Parameter는 선택적(Optional)이며, 누락 시 DTO의 기본값 적용
...
// property_id가 Query Parameter에 포함된 경우 filter_meta 검증
var propertyIdParam = pm.request.url.query.get('property_id');
if (propertyIdParam) {
    pm.test("FilterMeta exists for property-specific query", function () {
        var jsonData = pm.response.json();
        pm.expect(jsonData.filter_meta).to.not.be.null;
        pm.expect(jsonData.filter_meta.target_property_id).to.eql(propertyIdParam);
    });
}
```
...
**TC-2.1: 특정 매물 리뷰 조회**
```
GET {{base_url}}/api/v1/reviews?property_id=5d41402abc4b2a76b9719d911017c592&page=1&size=10&sort=latest
```
- Expected: 200 OK
...
**TC-2.3: 별점 높은순 정렬**
```
GET {{base_url}}/api/v1/reviews?page=1&size=10&sort=rating_desc
```
- Expected: 200 OK
...
**TC-2.4: 별점 낮은순 정렬**
```
GET {{base_url}}/api/v1/reviews?page=1&size=10&sort=rating_asc
```
- Expected: 200 OK
...
**TC-2.5: 잘못된 sort 값**
```
GET {{base_url}}/api/v1/reviews?page=1&size=10&sort=invalid_sort
```
- Expected: 400 Bad Request
- Error Field: `sort`
- Error Code: `Pattern`
...
```
GET {{base_url}}/api/v1/reviews?page=0&size=10&sort=latest
```
- Expected: 400 Bad Request
- Error Field: `page`
- Error Code: `Min`
...
```
GET {{base_url}}/api/v1/reviews?page=1&size=101&sort=latest
```
- Expected: 400 Bad Request
- Error Field: `size`
- Error Code: `Max`
...
| 2 | GET /api/v1/reviews?property_id=... | 200 OK, 작성한 리뷰 포함 확인 |
...
| 2 | GET /api/v1/reviews?property_id=A | property_id: A | avg_rating: 5.0, review_count: 1 |
| 3 | POST /api/v1/reviews | rating: 3 (동일 매물) | 201 Created |
| 4 | GET /api/v1/reviews?property_id=A | property_id: A | avg_rating: 4.0, review_count: 2 |
| 5 | POST /api/v1/reviews/update | rating: 5 → 1 | 200 OK |
| 6 | GET /api/v1/reviews?property_id=A | property_id: A | avg_rating 변경 확인 |
| 7 | DELETE /api/v1/reviews/{id} | - | 204 No Content |
| 8 | GET /api/v1/reviews?property_id=A | property_id: A | review_count 감소 확인 |
