...
│
├── Buffer Reads
│     When a required buffer is not in memory, the database reads the data 
│     block from disk into the buffer cache. (Cache Miss 발생 시 디스크에서 
│     블록을 읽어 Buffer Cache로 적재하는 과정)
│
└── Buffer Touch Counts
...
[Server Process가 Block X 요청]
    ...
    Buffer Cache 검색
           ...
           │                                    db block gets 증가)
           │                                   
           │                                   ※ consistent gets: Read-consistent view로
           │                                      블록 읽기 (SELECT 작업)
           │                                   ※ db block gets: Current mode로 블록 읽기
           │                                      (INSERT, UPDATE, DELETE 작업)
           │                                   → 본 문서의 N+1 문제는 SELECT 중심이므로
           │                                     consistent gets가 주요 관심 대상
           │
           └─── [Miss] ─────────────────────────────────┐
                 Block X가 메모리에 없음                 │
                                                        ▼
...
아래 임계값(10%)은 Oracle 커뮤니티에서 Buffer Cache Hit Ratio 90% 이상을 OLTP 환경의 양호한 수준으로 보는 오래된 관례(rule of thumb)에 기반한다. 단, 이는 워크로드 특성에 따라 유동적이며 절대적 기준이 아닌 참고 지표로 활용해야 한다. OLTP 환경에서는 95% 이상을 요구하기도 하고, DSS/DW 환경에서는 Full Table Scan이 정상이므로 낮은 Hit Ratio가 문제가 아닐 수 있다.
...
└── 나머지: 5,188ms (93.8%)     ← N+1 패턴으로 인한 25번의 개별 쿼리 순차 실행에서
                                   네트워크 왕복 시간 누적, 애플리케이션 처리

결론: DBMS 자체의 Buffer I/O가 병목이 아님
      25번의 쿼리가 각각 빠르게 처리되더라도 (Logical I/O 효율적)
      N+1 패턴의 순차 실행으로 인해 네트워크 왕복이 25회 누적되어 실제 병목 발생
```
