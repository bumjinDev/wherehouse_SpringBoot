...
│  │                                         │                           │
│  │ ※ Hot End 이동 시 Touch Count는 0으로  │ ◀── 리셋 메커니즘         │
│  │    리셋됨 (최근 구간 내 인기도 재측정)  │                           │
│  └─────────────────────────────────────────┘                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
│  [테이블 크기 > _small_table_threshold]                                 │
│                                                                         │
│     Oracle이 다음을 복합적으로 고려하여 동적 결정:                       │
│     - 해당 테이블 블록이 이미 Buffer Cache에 캐시된 비율                 │
│     - Buffer Cache의 현재 여유 공간 상태                                │
│                                                                         │
│     캐시 비율 높음 → Disk → Buffer Cache → Server Process              │
│     캐시 비율 낮음 → Disk → PGA (Direct Path Read) → Server Process    │
│                             │                                           │
...
| 1.2 | 2025-01-02 | 기술적 보완: Touch Count 리셋 메커니즘 추가(Hot End 이동 시 0으로 리셋), Direct Path Read 발생 조건 정정(캐시 비율에 따른 동적 결정), _small_table_threshold 조건 표현 정정 |
