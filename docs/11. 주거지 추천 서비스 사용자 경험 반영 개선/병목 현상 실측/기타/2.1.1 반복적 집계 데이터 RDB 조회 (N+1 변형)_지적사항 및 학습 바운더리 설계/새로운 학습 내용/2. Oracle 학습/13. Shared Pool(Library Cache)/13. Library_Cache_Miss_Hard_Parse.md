...
|------|-----------|------------|
| Hash 계산 & 검색 | ✓ 수행 | ✓ 수행 |
| Syntax Check | 건너뜀 (캐시된 Cursor 재사용) | ✓ 수행 |
| Semantic Check | 건너뜀 (커서 invalidation 여부만 확인) | ✓ 수행 (Dictionary Cache 접근) |
| 권한 검사 | 건너뜀 (커서 invalidation 여부만 확인) | ✓ 수행 (전체 권한 트리 해석) |
...
2~25번째: Soft Parse (Step 0에서 Hit → Step 1~6 생략, 기존 Cursor 재사용) ← Bind Variable 덕분
```
...
## 12. 부록: Oracle Cursor 개념

본 문서에서 "커서 invalidation 여부만 확인"이라는 표현이 등장한다. 이 섹션에서는 Oracle에서 말하는 Cursor가 무엇인지 명확히 정의한다.

### Cursor의 정의

Oracle에서 **Cursor**는 SQL 문장의 실행 컨텍스트를 추상화한 개념이다. 두 가지 레벨로 구분된다.

### 1. Shared Cursor (Library Cache 레벨)

Library Cache에 저장되는 **SQL 문장의 파싱된 표현 전체**를 가리킨다. Shared SQL Area와 사실상 동의어다.

```
Shared Cursor가 포함하는 것:
├── SQL 텍스트 (원본)
├── Parse Tree (Syntax Check 결과물)
├── 객체 참조 정보 (Semantic Check 결과물: 어떤 테이블, 어떤 컬럼)
├── 권한 검사 결과
├── Execution Plan (Optimizer 결과물)
├── Row Source Tree (Row Source Generator 결과물)
└── 의존성 목록 (이 커서가 의존하는 객체들: employees 테이블, emp_dept_ix 인덱스 등)
```

본 문서에서 "커서 invalidation 여부만 확인"이라고 할 때의 커서가 이것이다. DDL(ALTER TABLE, DROP INDEX 등)이나 REVOKE가 발생하면, 해당 객체에 의존하는 Shared Cursor들이 **invalidate** 상태로 표시된다. Soft Parse 시점에 이 상태를 확인하여, invalidate되었으면 Hard Parse로 전환한다.

### 2. Session Cursor (세션 레벨)

개별 세션이 SQL을 실행할 때 사용하는 **핸들/포인터**다. PGA의 Private SQL Area에 위치한다.

```
Session Cursor가 포함하는 것:
├── Shared Cursor를 가리키는 포인터
├── Bind Variable 값 (세션마다 다름)
├── 실행 상태 (현재 어느 row까지 fetch했는지)
└── 세션별 런타임 정보
```

JDBC의 `PreparedStatement`가 내부적으로 Session Cursor를 할당받는다. `prepareStatement()` 호출 시 Session Cursor가 열리고, 이 Session Cursor는 Library Cache의 Shared Cursor를 참조한다.
...
│                         SGA - Library Cache                      │
│  ┌─────────────────────────────────────────────────────────────┐│
│  │              Shared Cursor (Shared SQL Area)                 ││
│  │  SQL: SELECT * FROM employees WHERE dept_id = :1            ││
│  │  Parse Tree: [...]                                          ││
│  │  Execution Plan: INDEX RANGE SCAN → TABLE ACCESS            ││
│  │  Dependencies: [employees, emp_dept_ix]                     ││
...
│ │            │ │      │ │            │ │      │ │            │ │
│ │ bind: 10   │ │      │ │ bind: 20   │ │      │ │ bind: 30   │ │
│ │ row: 5/100 │ │      │ │ row: 1/50  │ │      │ │ row: 완료  │ │
│ └────────────┘ │      │ └────────────┘ │      │ └────────────┘ │
└────────────────┘      └────────────────┘      └────────────────┘
```
...
- **Shared Cursor는 1개** (Library Cache에서 공유)
- **Session Cursor는 3개** (각 세션의 PGA에 별도 존재)

### Soft Parse와 Cursor의 관계

Soft Parse가 발생하면:

1. Hash 계산 → Library Cache에서 **Shared Cursor** 검색
2. Hit 발생 → Shared Cursor의 **Status**가 VALID인지 확인
3. VALID면 → Session Cursor가 해당 Shared Cursor를 참조하도록 연결
4. Execute 단계로 진입

"커서 invalidation 여부만 확인"이란, Shared Cursor의 Status 필드를 체크하는 것이다. 이 확인은 O(1) 연산이므로, Syntax/Semantic/권한 검사를 다시 수행하는 것과는 비용 차원이 다르다.
