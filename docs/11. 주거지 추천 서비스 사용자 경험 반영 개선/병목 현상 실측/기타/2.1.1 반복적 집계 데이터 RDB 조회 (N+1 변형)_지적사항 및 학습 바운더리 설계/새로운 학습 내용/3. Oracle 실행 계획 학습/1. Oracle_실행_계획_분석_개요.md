...
**CONCATENATION**: IN 절이 OR로 분해되어 여러 쿼리 블록으로 나뉘었을 때 나타난다. Wherehouse 프로젝트에서 Bulk Fetch가 비효율적이었던 이유가 바로 이것이다. IN 절에 1000개 초과의 값이 포함되면 Hibernate가 OR로 분해하고, Oracle이 CONCATENATION 실행 계획을 수립한다. 이 패턴이 나타나면 IN 절을 1000개 이하로 분할하는 Chunk 방식이 해결책이 된다.
