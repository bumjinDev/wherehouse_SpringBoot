# Oracle IN 절과 CONCATENATION 심층 분석
...
1. 다른 SQL 문장을 위한 공간이 부족해진다.
2. Library Cache LRU 알고리즘에 의해 유용한 캐시 엔트리가 밀려난다.
3. 전체 시스템의 Hard Parse 비율이 증가한다.

### 제한 존재 이유 3: 네트워크 패킷 크기 고려

SQL 문장은 클라이언트에서 서버로 네트워크를 통해 전송된다. Oracle Net의 SDU(Session Data Unit) 기본값은 Oracle 11g까지 8KB, 12c R2 이후 64KB다. 8KB 기준으로 계산하면:
...
## CONCATENATION 비효율성의 구체적 분석

### "각 분기가 독립적으로 실행된다"의 의미
...
|------|-----------------|----------------------|
| 독립 최적화되는 쿼리 블록 수 | 1개 | 2개 |
| Row Source 생성 횟수 | 1세트 | 2세트 |
| 메모리 할당 | 1세트 | 2세트 |
...
### "별도의 Operation으로 처리되어 오버헤드가 증가한다"의 의미
...
### "결과 병합 과정에서 중복 제거가 필요할 수 있다"의 의미
...
## CONCATENATION이 OR 분기를 별도 쿼리 블록으로 취급하는 이유

### OR 조건의 본질적 특성: 각 OR 분기가 서로 다른 최적 접근 경로를 가질 수 있다
...
```
[1단계: Hibernate의 개입 (Oracle 전송 전)]
- 파라미터 개수 확인
- 1000개 초과 감지 시 여러 IN 절로 분할하고 OR로 연결
- 변환된 SQL을 Oracle에 전송

[2단계: Oracle SQL Parser]
- IN 절 1000개 제한 검사 (이미 분할되어 있으므로 통과)
- 파스 트리 생성

[3단계: Oracle Query Transformer]
- OR 조건 감지
- OR Expansion 변환 규칙 적용
- 각 OR 분기를 별도 쿼리 블록으로 분리

[4단계: Oracle Optimizer]
...
참고: 만약 Hibernate 없이 순수 Oracle SQL에서 1000개를 초과하는 IN 절을 직접 실행하면, 1단계(SQL Parser)에서 즉시 ORA-01795 오류가 발생한다.
...
1. **1000개 제한 위반**: 변환 결과가 다시 1000개를 초과하므로 원점으로 돌아간다.
2. **Query Transformer의 설계 원칙**: Query Transformer는 쿼리를 확장(Expansion)하는 방향으로 설계되었다. 축소(Contraction) 변환은 제한적으로만 수행된다. IN 절 병합은 축소 변환에 해당하며, 1000개 제한과 충돌하므로 수행되지 않는다.
3. **변환 규칙의 일반성**: OR Expansion은 일반적인 OR 조건에 대해 동작한다. "동일 컬럼 IN 절의 OR 연결"이라는 특수 케이스를 별도로 처리하는 규칙이 없다.
...
## Wherehouse 프로젝트에서 실제로 문제가 되는 것

"실행 계획 폭발"은 해당되지 않는다. 실제 문제는 다음 두 가지다:

### 1. CONCATENATION으로 인한 Row Source 증가
...
| 단일 쿼리 내 Row Source 수 | 약 30개 (동시 존재) | 3~4개 (쿼리당) |
...
## CONCATENATION이 별도 쿼리 블록으로 취급하는 이유 요약
...
### INDEX SCAN → TABLE ACCESS BY INDEX ROWID 흐름
...
**1000개 초과 IN 절 (OR 분해 발생 케이스)**의 실행 계획을 캡처하면 CONCATENATION이 나타나는 것을 확인할 수 있다. Hibernate가 생성한 실제 SQL(1000개 + 660개 OR 연결)의 실행 계획을 캡처할 수 있는가?

---

## 실행 계획 계층 구조의 설계 원리

### 실행 계획은 Row Source Tree다
...
INLIST ITERATOR는 IN 절의 각 값마다 이 과정을 반복한다. 값을 바꿀 때 하위 Row Source를 완전히 close/open하는 것이 아니라, restart 호출로 하위 Row Source를 재초기화한다. 이것이 INLIST ITERATOR가 효율적인 이유다.
...
인덱스 접근 후 추가 필터링 조건이다. 인덱스나 테이블에서 읽어온 행 중 조건에 맞지 않는 행을 제거한다.
...
#### 2. 인덱스가 있어도 Optimizer가 Full Scan이 효율적이라 판단
...
### TABLE ACCESS FULL이 오히려 효율적인 경우

#### 1. 반환 행이 테이블의 상당 부분일 때
...
### Wherehouse 프로젝트에서 TABLE ACCESS FULL이 나타나면
...
### INDEX SCAN vs TABLE ACCESS FULL 요약
