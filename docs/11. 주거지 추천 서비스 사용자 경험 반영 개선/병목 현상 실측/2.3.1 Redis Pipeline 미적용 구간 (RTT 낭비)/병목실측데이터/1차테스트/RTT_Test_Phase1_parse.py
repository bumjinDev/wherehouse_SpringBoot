...
def export_to_xlsx(sequential: List[SequentialMetric],
    ...
    desc_header_fill = PatternFill('solid', fgColor='70AD47')  # 녹색 (설명 테이블용)
    
    def apply_header_style(sheet, row_num, col_count, fill=None):
    ...
    if sequential:
        ...
        data_end_row = len(sequential) + 1
        apply_data_style(ws1, 2, data_end_row, len(headers))
        
        # ========================================
        # 컬럼 설명 테이블 추가 (데이터 테이블 아래)
        # ========================================
        desc_start_row = data_end_row + 3  # 2행 공백 후 시작
        
        # 설명 테이블 제목
        ws1.cell(row=desc_start_row, column=1, value="📋 컬럼 설명")
        ws1.cell(row=desc_start_row, column=1).font = Font(bold=True, size=12)
        
        desc_header_row = desc_start_row + 1
        ws1.append([])  # 빈 행
        ws1.cell(row=desc_header_row, column=1, value="컬럼명")
        ws1.cell(row=desc_header_row, column=2, value="설명")
        ws1.cell(row=desc_header_row, column=3, value="단위")
        apply_header_style(ws1, desc_header_row, 3, desc_header_fill)
        
        column_descriptions = [
            ('Timestamp', '로그 기록 시간', '-'),
            ('Thread', 'Tomcat 스레드 이름 (예: http-nio-8080-exec-1)', '-'),
            ('District', '검색 대상 지역구명', '-'),
            ('Commands', '실행된 Redis 명령 수 (1~3, -1은 에러)', '개'),
            ('MethodTime(ms)', '메소드 총 실행 시간 (진입~반환)', 'ms'),
            ('CmdLatency(ms)', '3개 Redis 명령의 Latency 합계', 'ms'),
            ('IO_Ratio(%)', '(CmdLatency / MethodTime) × 100, I/O 바운드 정도', '%'),
            ('NonIO(ms)', '비-I/O 시간 (스트림 변환, 교집합 계산 등)', 'ms'),
            ('Cmd1(ms)', 'Redis 보증금 조회 (idx:deposit:{district}) Latency', 'ms'),
            ('Cmd1_Count', '보증금 조회 결과 건수', '건'),
            ('Cmd2(ms)', 'Redis 월세금 조회 (idx:monthlyRent:{district}:월세) Latency', 'ms'),
            ('Cmd2_Count', '월세금 조회 결과 건수', '건'),
            ('Cmd3(ms)', 'Redis 평수 조회 (idx:area:{district}:월세) Latency', 'ms'),
            ('Cmd3_Count', '평수 조회 결과 건수', '건'),
            ('Status', '처리 결과 (SUCCESS, EARLY_RETURN_*, ERROR)', '-'),
            ('Intersection', '3개 ZSet 교집합 결과 매물 수', '건'),
        ]
        
        for i, (col_name, desc, unit) in enumerate(column_descriptions):
            row_num = desc_header_row + 1 + i
            ws1.cell(row=row_num, column=1, value=col_name)
            ws1.cell(row=row_num, column=2, value=desc)
            ws1.cell(row=row_num, column=3, value=unit)
        
        desc_end_row = desc_header_row + len(column_descriptions)
        apply_data_style(ws1, desc_header_row + 1, desc_end_row, 3)
        
        # Status 값 설명 추가
        status_start_row = desc_end_row + 3
        ws1.cell(row=status_start_row, column=1, value="📋 Status 값 설명")
        ws1.cell(row=status_start_row, column=1).font = Font(bold=True, size=12)
        
        status_header_row = status_start_row + 1
        ws1.cell(row=status_header_row, column=1, value="Status 값")
        ws1.cell(row=status_header_row, column=2, value="설명")
        apply_header_style(ws1, status_header_row, 2, desc_header_fill)
        ...
        for i, (status_val, desc) in enumerate(status_descriptions):
            row_num = status_header_row + 1 + i
            ws1.cell(row=row_num, column=1, value=status_val)
            ws1.cell(row=row_num, column=2, value=desc)
        
        status_end_row = status_header_row + len(status_descriptions)
        apply_data_style(ws1, status_header_row + 1, status_end_row, 2)
    ...
    print(f"   - Sequential_Metrics: {len(sequential)}건 (컬럼 설명 포함)")
