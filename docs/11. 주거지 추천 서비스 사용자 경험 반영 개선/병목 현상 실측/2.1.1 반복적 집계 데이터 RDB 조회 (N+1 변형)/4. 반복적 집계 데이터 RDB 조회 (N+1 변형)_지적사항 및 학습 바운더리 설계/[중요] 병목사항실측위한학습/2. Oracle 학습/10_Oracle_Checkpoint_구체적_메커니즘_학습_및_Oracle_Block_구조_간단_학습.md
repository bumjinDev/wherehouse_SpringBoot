...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│              물리적 I/O 위치 vs 논리적 기록 순서                              │
│                                                                             │
│  Checkpoint Queue 상태:                                                     │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Block A: File 1, Block 500  (SCN 1000)  ← head (가장 오래됨)        │   │
│  │ Block B: File 3, Block 12   (SCN 1050)                              │   │
│  │ Block C: File 1, Block 501  (SCN 1100)                              │   │
│  │ Block D: File 2, Block 999  (SCN 1150)  ← tail (최근)               │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  DBWR 처리 순서:                                                            │
│  ─────────────────                                                         │
│  1단계: Block A (File 1, Block 500) 기록  ← 물리적으로 랜덤 위치           │
│  2단계: Block B (File 3, Block 12) 기록   ← 다른 파일, 다른 위치           │
│  3단계: Block C (File 1, Block 501) 기록  ← 또 다른 위치                   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  물리적 I/O 패턴: 랜덤 (디스크 여러 위치에 흩어져 기록)              │   │
│  │  논리적 처리 순서: 순차 (Checkpoint Queue head부터 SCN 순서대로)     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  핵심:                                                                      │
│  DBWR이 Block C(SCN 1100)를 기록 완료한 시점에,                            │
│  Block A(SCN 1000), Block B(SCN 1050)는 반드시 이미 기록 완료 상태다.      │
│  왜냐하면 Checkpoint Queue에서 A, B가 C보다 앞에 있었고,                    │
│  head부터 순차 처리되어 이미 큐에서 제거되었기 때문이다.                    │
└─────────────────────────────────────────────────────────────────────────────┘
```

**"중간이 붕 뜨는" 시나리오가 불가능한 구조적 이유:**
...
│  가정: DBWR가 "랜덤 순서"로 기록                                            │
│  ─────────────────────────────────                                         │
│  Redo Log 순서: SCN 100 → 101 → 102 → 103 → 104                            │
│                                                                             │
│  DBWR가 임의로 기록했다면:                                                  │
│  - Block C (SCN 102) ✓ 기록됨                                              │
│  - Block A (SCN 100) ✗ 아직 미기록                                         │
│  - Block E (SCN 104) ✓ 기록됨                                              │
│                                                                             │
│  Checkpoint Position = 104로 설정하면?                                      │
│  → SCN 100, 101, 103은 "붕 뜬다"                                           │
│  → 장애 시 복구 누락 발생!                                                  │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                   실제 Oracle 설계 (붕 뜸 원천 차단)                         │
│                                                                             │
│  Checkpoint Queue의 순차 처리 보장:                                         │
│  ─────────────────────────────────                                         │
│  Block C(SCN 102)가 기록되었다면,                                           │
│  Block A(SCN 100), Block B(SCN 101)는 반드시 이미 기록 완료.                │
│                                                                             │
│  이유: Checkpoint Queue는 SCN 순서로 정렬되어 있고,                         │
│        DBWR은 head(가장 오래된 것)부터만 처리하기 때문.                      │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  Checkpoint Position = X가 Control File에 기록되어 있다면,          │   │
│  │  SCN < X인 모든 변경은 반드시 Data Files에 존재한다.                 │   │
│  │                                                                      │   │
│  │  이것이 Oracle Checkpoint 설계의 핵심 불변식(Invariant)이다.         │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
실제 DBWR은 성능을 위해 여러 블록을 배치로 묶어 비동기 I/O를 수행한다. 이 경우 배치 내 블록들의 기록 완료 순서가 SCN 순서와 다를 수 있다.
...
│  DBWR 배치 기록 요청:                                                       │
│  ─────────────────────                                                     │
│  배치 1: Block A(SCN 100), B(SCN 101), C(SCN 102) 동시 기록 요청            │
│                                                                             │
│  장애 발생 시점의 실제 Data File 상태:                                      │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Block A(SCN 100): ✓ 기록 완료                                       │   │
│  │ Block B(SCN 101): ✗ 미완료 (I/O 진행 중 장애)                       │   │
│  │ Block C(SCN 102): ✓ 기록 완료 (배치 내에서 먼저 완료됨)             │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  이 상태에서 "중간이 붕 뜬" 것처럼 보인다.                                  │
│  SCN 101만 미기록, SCN 100과 102는 기록됨.                                  │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                    안전장치 1: CKPT의 보수적 업데이트 정책                    │
│                                                                             │
│  CKPT 프로세스는 배치 기록이 완전히 완료되기 전에는                          │
│  Control File의 Checkpoint Position을 업데이트하지 않는다.                  │
│                                                                             │
│  위 시나리오에서:                                                           │
│  - 배치 1이 완료되지 않았으므로 Checkpoint Position = 100 이전 값 유지      │
│  - 장애 후 Recovery는 SCN 100 이전부터 시작                                 │
│  - Block A, B, C 모두 Recovery 범위에 포함됨                                │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                    안전장치 2: Redo 적용의 멱등성(Idempotency)               │
│                                                                             │
│  Recovery 시 Block A, C는 이미 Data File에 최신 값이 있지만,                │
│  Redo를 다시 적용해도 문제없다.                                             │
│                                                                             │
│  Redo 적용 로직:                                                            │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │      Redo 적용;  // 실제로 변경 필요                                 │   │
│  │  } else {                                                           │   │
│  │      스킵;       // 이미 최신 상태, 적용해도 동일 결과               │   │
│  │  }                                                                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  결과:                                                                      │
│  - Block A(SCN 100): 스킵 또는 동일 값 재적용 (무해)                        │
│  - Block B(SCN 101): 실제 복구됨                                            │
│  - Block C(SCN 102): 스킵 또는 동일 값 재적용 (무해)                        │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                          3중 안전장치 요약                                   │
│                                                                             │
│  1. Checkpoint Queue 순차 처리                                              │
│     → 정상 상황에서 "붕 뜸" 자체가 발생하지 않음                            │
│                                                                             │
│  2. CKPT의 보수적 업데이트                                                  │
│     → 배치 완료 전 장애 시, 이전 Checkpoint Position 유지                   │
│     → Recovery 범위가 더 넓어져 안전측으로 동작                             │
│                                                                             │
│  3. Redo 적용의 멱등성                                                      │
│     → 이미 기록된 블록에 Redo 재적용해도 무결성 유지                        │
│     → 중복 적용이 해가 되지 않음                                            │
└─────────────────────────────────────────────────────────────────────────────┘
```
