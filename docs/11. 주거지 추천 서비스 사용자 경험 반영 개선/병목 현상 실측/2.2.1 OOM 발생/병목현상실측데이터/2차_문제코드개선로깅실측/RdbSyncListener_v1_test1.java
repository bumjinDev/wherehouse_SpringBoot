...
//        saveMonthlyPropertiesToRdb(event.getMonthlyProperties());
        ...
        log.info("[PERF:DBLOAD:CHARTER] thread={} | phase=START | ts={}", threadName, charterStartTs);

        List<PropertyCharter> charterEntities = propertyCharterRepository.findAll();
        // ▶▶▶ 브레이크포인트 #1: 이 라인에서 정지 → VisualVM 힙 덤프 (전세만 적재 상태) ◀◀◀

        long charterEndTs = System.currentTimeMillis();
        log.info("[PERF:DBLOAD:CHARTER] thread={} | phase=END | ts={} | count={} | elapsed_ms={}",
                threadName, charterEndTs, charterEntities.size(), (charterEndTs - charterStartTs));

        // === 월세 데이터 로드 ===
        long monthlyStartTs = System.currentTimeMillis();
        log.info("[PERF:DBLOAD:MONTHLY] thread={} | phase=START | ts={}", threadName, monthlyStartTs);

        List<PropertyMonthly> monthlyEntities = propertyMonthlyRepository.findAll();
        // ▶▶▶ 브레이크포인트 #2: 이 라인에서 정지 → VisualVM 힙 덤프 (전세+월세 모두 적재 상태) ◀◀◀

        long monthlyEndTs = System.currentTimeMillis();
        log.info("[PERF:DBLOAD:MONTHLY] thread={} | phase=END | ts={} | count={} | elapsed_ms={}",
                threadName, monthlyEndTs, monthlyEntities.size(), (monthlyEndTs - monthlyStartTs));
        ...
        // === 전세 Entity → DTO 변환 ===
        long transformCharterStartTs = System.currentTimeMillis();
        log.info("[PERF:TRANSFORM:CHARTER] thread={} | phase=START | ts={}", threadName, transformCharterStartTs);

        List<Property> charterProperties = charterEntities.stream()
                .map(this::convertCharterEntityToProperty)
                .collect(Collectors.toList());

        long transformCharterEndTs = System.currentTimeMillis();
        log.info("[PERF:TRANSFORM:CHARTER] thread={} | phase=END | ts={} | count={} | elapsed_ms={}",
                threadName, transformCharterEndTs, charterProperties.size(), (transformCharterEndTs - transformCharterStartTs));

        // === 월세 Entity → DTO 변환 ===
        long transformMonthlyStartTs = System.currentTimeMillis();
        log.info("[PERF:TRANSFORM:MONTHLY] thread={} | phase=START | ts={}", threadName, transformMonthlyStartTs);

        List<Property> monthlyProperties = monthlyEntities.stream()
                .map(this::convertMonthlyEntityToProperty)
                .collect(Collectors.toList());

        long transformMonthlyEndTs = System.currentTimeMillis();
        log.info("[PERF:TRANSFORM:MONTHLY] thread={} | phase=END | ts={} | count={} | elapsed_ms={}",
                threadName, transformMonthlyEndTs, monthlyProperties.size(), (transformMonthlyEndTs - transformMonthlyStartTs));
        ...
        log.info("====================================================================");
        log.info(">>> [1차 테스트] 측정 결과 요약 <<<");
        log.info("  DBLOAD:CHARTER    = {} ms ({}건)", (charterEndTs - charterStartTs), charterEntities.size());
        log.info("  DBLOAD:MONTHLY    = {} ms ({}건)", (monthlyEndTs - monthlyStartTs), monthlyEntities.size());
        log.info("  TRANSFORM:CHARTER = {} ms", (transformCharterEndTs - transformCharterStartTs));
        log.info("  TRANSFORM:MONTHLY = {} ms", (transformMonthlyEndTs - transformMonthlyStartTs));
