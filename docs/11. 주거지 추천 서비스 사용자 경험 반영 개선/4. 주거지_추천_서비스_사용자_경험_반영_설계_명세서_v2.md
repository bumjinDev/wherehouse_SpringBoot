...
* Public API: 인증 불필요 (리뷰 조회, 추천 요청, 매물 검색)
* Protected API: Authorization: Bearer {Access_Token} 헤더 필수 (리뷰 작성, 수정, 삭제)

**에러 응답**: GlobalExceptionHandlerReview를 통해 표준 ErrorResponseDto 포맷 반환.

**컨트롤러 구조**: 리뷰 시스템은 책임 분리 원칙에 따라 다음과 같이 5개의 컨트롤러로 분리 구현되어 있다.

| 컨트롤러 | 패키지 | 책임 |
|----------|--------|------|
| ReviewQueryController | com.wherehouse.review.controller | 리뷰 조회 (목록, 상세) |
| ReviewWriteController | com.wherehouse.review.controller | 리뷰 CUD (작성, 수정, 삭제) |
| ReviewBoardViewController | com.wherehouse.review.controller | JSP 뷰 렌더링 (게시판 UI) |
| PropertySearchController | com.wherehouse.review.controller | 매물 검색 (자동완성) |
| GlobalExceptionHandlerReview | com.wherehouse.review.controller | 리뷰 도메인 예외 처리 |

### 6.2 리뷰 작성 API : 주거지 추천 결과로 확인된 개별 매물에 대한 리뷰 작성

| 항목 | 내용 |
|------|------|
| **기능 설명** | 사용자가 추천 결과로 확인된 개별 특정 매물에 대해서 별점과 텍스트 리뷰를 등록한다. 작성 성공 시 RDB에 저장되고 통계 데이터가 갱신된다. |
| **HTTP Method** | POST |
| **Endpoint URL** | /api/v1/reviews |
| **인증 여부** | 필수 (Protected) |
| **담당 컨트롤러** | ReviewWriteController |

- 추천 서비스 결과가 아닌, 별도로 리뷰 시스템 자체에서 매물을 검색해서 그 매물에 대한(매물 검색에 대한 MD 조회 후 JS에 반환 기능 개발 및 그에 대한 별도 API 설계 필요) 리뷰 작성 기능은 현재 개발하지 않는다.
...
| 필드 | 타입 | 필수 | 설명 | 예시 |
|------|------|------|------|------|
| propertyId | String | ✓ | 매물 식별자 (32자 MD5 Hash), 리뷰 요청 시 자동 반영. | "5d41402abc..." |
| rating | Integer | ✓ | 별점 (1~5 정수) | 5 |
| content | String | ✓ | 리뷰 내용 (20자 이상 1000자 이하) | "채광이 좋고..." |

**userId 처리**: Request Body에 포함하지 않는다. 서버에서 `@AuthenticationPrincipal` 어노테이션을 통해 인증된 사용자 정보로부터 자동 주입한다. 이는 클라이언트 조작 방지 및 보안 강화를 위한 설계이다.
...
### 6.3 리뷰 목록 조회 API : 리뷰 시스템(게시판)에서 매물 명으로 검색 혹은 전체 매물 목록 조회

| 항목 | 내용 |
|------|------|
| **기능 설명** | 조건에 따라 리뷰 목록을 페이징 조회한다. |
| **HTTP Method** | GET |
| **Endpoint URL** | /api/v1/reviews/list |
| **인증 여부** | 불필요 (Public) |
| **담당 컨트롤러** | ReviewQueryController |

#### 6.3.1 요청 (Request)

**Query Parameters** (GET 요청이므로 Request Body가 아닌 Query String으로 전달, `@ModelAttribute`를 통해 ReviewListRequestDto로 바인딩)

| 필드 | 타입 | 필수 | 설명 | 동작 방식 |
|----------|------|------|------|----------|
| propertyId | String | ✗ | 매물 식별자 (MD5) | 사용자가 UI에서 매물명을 입력하는 과정에서 발생하는 실시간 키 입력 이벤트마다 별도의 자동완성 API(6.7 매물 검색 API 참조)를 호출하여 팝업을 갱신하고, 여기서 사용자가 특정 매물을 선택하면 획득한 MD5 식별자를 기반으로 해당 매물의 리뷰를 조회하며, 입력값이 비어있을 경우 전체 리뷰 목록을 반환한다. |
| page | Integer | ✗ | 페이지 번호 | Default: 0 (0-indexed) |
| size | Integer | ✗ | 페이지 크기 | Default: 10 |
| sort | String | ✗ | 정렬 기준 | latest(최신순, 기본값), rating_desc(별점 높은순), rating_asc(별점 낮은순) |
| keyword | String | ✗ | 검색어 | 리뷰 내용 검색, 리뷰 테이블과 연결된 키워드 테이블 내 조회할 검색어 |

**Request 예시**

```
GET /api/v1/reviews/list?propertyId=5d41402abc4b2a76b9719d911017c592&page=0&size=10&sort=latest&keyword=전망
```
...
| 필드 | 타입 | 설명 |
|------|------|------|
| filterMeta | Object | 필터링 메타 정보 (특정 매물 조회인 propertyId 요청 시에만 포함) |
| filterMeta.targetPropertyId | String | 조회 대상 매물 ID |
| filterMeta.propertyAvgRating | Double | 해당 매물의 평균 별점 |
...
| reviews[].content | String | 리뷰 본문 (요약) |
| totalPages | Integer | 전체 페이지 수 |
| totalElements | Long | 전체 리뷰 수 |
...
{
  "filterMeta": {
    "targetPropertyId": "5d41402abc4b2a76b9719d911017c592",
...
| userId | String | 작성자 ID (마스킹 처리) |
| rating | Integer | 별점 |
| content | String | 리뷰 본문 (전체) |
| tags | Array\<String\> | 자동 추출된 키워드 태그 목록 |
| createdAt | String | 작성 일시 (ISO 8601) |
| updatedAt | String | 수정 일시 (ISO 8601, 수정된 경우) |
...
```json
{
  ...
  "userId": "user****",
  "rating": 5,
  "content": "남향이라 채광이 정말 좋고, 관리비도 저렴한 편입니다. 다만 주차 공간이 조금 협소해요. (전체 원문 반환)...",
  "tags": ["채광", "관리비", "주차"],
  "createdAt": "2025-11-27T10:30:00",
...
| 항목 | 내용 |
|------|------|
| **기능 설명** | 사용자가 본인이 작성한 리뷰를 수정한다. 수정 시 키워드 재추출 및 통계 데이터가 재계산된다. |
| **HTTP Method** | POST |
| **Endpoint URL** | /api/v1/reviews/update |
...
| rating | Integer | ✓ | 별점 (1~5 정수) |
| content | String | ✓ | 리뷰 내용 (20자 이상 1000자 이하) |

**userId 처리**: Request Body에 포함하지 않는다. 서버에서 `@AuthenticationPrincipal` 어노테이션을 통해 인증된 사용자 정보로부터 자동 주입하며, 리뷰 작성자와 현재 인증 사용자의 일치 여부를 검증한다.
...
| 항목 | 내용 |
|------|------|
| **기능 설명** | 사용자가 리뷰 게시판에서 매물명을 입력할 때 실시간 자동완성 기능을 제공한다. 입력 패턴에 매칭되는 매물 목록과 해당 MD5 식별자를 반환한다. |
| **HTTP Method** | GET |
| **Endpoint URL** | /api/v1/properties/search |
| **인증 여부** | 불필요 (Public) |
| **담당 컨트롤러** | PropertySearchController |
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| keyword | String | ✓ | 검색 키워드 (매물명 일부) |

**Request 예시**

```
GET /api/v1/properties/search?keyword=래미안
```
...
| 필드 | 타입 | 설명 |
|------|------|------|
| results | Array | 매칭된 매물 목록 |
| results[].propertyId | String | 매물 식별자 (MD5 Hash) |
| results[].propertyName | String | 매물명 (아파트명) |
| results[].address | String | 매물 주소 |
...
```json
{
  "results": [
    ...
    {
      "propertyId": "7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e",
      "propertyName": "래미안퍼스티지",
...
### 6.8 추천 결과 조회 API (변경사항)

기존 추천 API (/api/recommendations/*)의 응답 DTO 구조가 변경된다. 리뷰 시스템 도입으로 인해 매물별 리뷰 통계 정보가 추천 결과에 포함된다.

**변경된 Response DTO 구조 (TopPropertyDto)**

| 필드 | 타입 | 설명 | 비고 |
|------|------|------|------|
| propertyId | String | 매물 식별자 | 기존 랜덤 UUID 생성에서 MD5 Hash로 변경됨 |
| reviewCount | Integer | 리뷰 개수 | 신규 추가 |
| avgRating | Double | 평균 별점 | 신규 추가 |
| finalScore | Double | 최종 추천 점수 | 산출 로직 변경 (리뷰 가중치 반영) |

### 6.9 공통 에러 코드 (Error Codes)

시스템 전반에서 발생하는 주요 에러 코드는 다음과 같다. GlobalExceptionHandlerReview에서 처리된다.
...
| 속성 | 값 |
|------|------|
| **Key Pattern** | `stats:{id}` |
| **대응 RDB** | REVIEW_STATISTICS |
| **생성 주체** | RdbSyncListener (배치) |
| **갱신 주체** | ReviewService (실시간) |
| **갱신 시점** | 리뷰 CUD 트랜잭션 커밋 직후 |
...
**조회 예시**
```redis
HGETALL stats:5d41402abc4b2a76b9719d911017c592
HMGET stats:a1b2c3d4... reviewCount avgRating
```
...
**[구현 완료]** 모든 리뷰 관련 DTO가 구현되어 운영 중이다.

### 7.3.4.1 Request DTO

#### ReviewCreateRequestDto (리뷰 작성 요청)

**패키지**: `com.wherehouse.review.dto`

사용자가 리뷰를 작성할 때 전달하는 요청 객체. userId는 Request Body에 포함하지 않으며, 서버에서 `@AuthenticationPrincipal` 어노테이션을 통해 인증된 사용자 정보로부터 자동 주입한다.
...
#### ReviewListRequestDto (리뷰 목록 조회 요청)

**패키지**: `com.wherehouse.review.dto`

리뷰 목록 조회 시 Query Parameter를 바인딩하는 객체. `@ModelAttribute`를 통해 Query String에서 자동 바인딩된다.
...
public class ReviewListRequestDto {
    ...
    private String propertyId;          // 매물 ID (선택, MD5 Hash)
    
    @Builder.Default
    private Integer page = 0;           // 페이지 번호 (0-indexed)
    
    @Builder.Default
    private Integer size = 10;          // 페이지 크기
    
    @Builder.Default
    private String sort = "latest";     // 정렬 기준 (latest, rating_desc, rating_asc)
    
    private String keyword;             // 검색 키워드 (선택)
}
```

**API Endpoint**: `GET /api/v1/reviews/list`

**Request 예시**
```
GET /api/v1/reviews/list?propertyId=5d41402abc4b2a76b9719d911017c592&page=0&size=10&sort=latest
```

---

#### ReviewUpdateRequestDto (리뷰 수정 요청)

**패키지**: `com.wherehouse.review.dto`

사용자가 본인의 리뷰를 수정할 때 전달하는 요청 객체. userId는 서버에서 `@AuthenticationPrincipal`을 통해 주입되며, 작성자 본인 여부를 검증한다.
...
public class ReviewUpdateRequestDto {
    ...
    private Long reviewId;              // 수정할 리뷰 ID
...
**API Endpoint**: `POST /api/v1/reviews/update`
...
#### ReviewListResponseDto (리뷰 목록 조회 응답)

**패키지**: `com.wherehouse.review.dto`

리뷰 목록 조회 API의 응답 객체. 특정 매물 조회 시 filterMeta를 포함하고, 페이지네이션 정보를 함께 반환한다.
...
public class ReviewListResponseDto {
    ...
    private FilterMetaDto filterMeta;           // 필터링 메타 정보 (특정 매물 조회 시)
    private List<ReviewSummaryDto> reviews;     // 리뷰 목록 (요약)
...
#### FilterMetaDto (필터링 메타 정보)

**패키지**: `com.wherehouse.review.dto`

특정 매물의 리뷰 조회 시 반환되는 메타 정보.
...
public class FilterMetaDto {
    ...
    private String targetPropertyId;    // 조회 대상 매물 ID (MD5 Hash)
    private Double propertyAvgRating;   // 해당 매물의 평균 별점
}
```

---

#### ReviewSummaryDto (리뷰 요약)

**패키지**: `com.wherehouse.review.dto`

리뷰 목록 조회 시 반환되는 개별 리뷰 요약 정보.
...
public class ReviewSummaryDto {
    ...
    private Long reviewId;              // 리뷰 ID (상세 조회 Key)
    private String propertyId;          // 매물 ID (MD5 Hash)
    private String propertyName;        // 아파트/매물명
    private String userId;              // 작성자 ID (마스킹 처리)
    private Integer rating;             // 별점 (1~5)
    private String content;             // 리뷰 본문 (요약, 일정 길이로 자름)
}
```

---

#### ReviewDetailDto (리뷰 상세)

**패키지**: `com.wherehouse.review.dto`

리뷰 단건 상세 조회 API의 응답 객체. 전체 본문과 키워드 태그, 작성/수정 일시를 포함한다.
...
public class ReviewDetailDto {
    ...
    private String userId;              // 작성자 ID (마스킹 처리)
    private Integer rating;             // 별점 (1~5)
    private String content;             // 리뷰 본문 (전체)
    private List<String> tags;          // 자동 추출된 키워드 태그 목록
    private String createdAt;           // 작성 일시 (ISO 8601)
    private String updatedAt;           // 수정 일시 (ISO 8601, 수정된 경우)
}
```

**API Endpoint**: `GET /api/v1/reviews/{reviewId}`
...
#### ReviewUpdateResponseDto (리뷰 수정 응답)

**패키지**: `com.wherehouse.review.dto`

리뷰 수정 성공 후 반환되는 응답 객체.
...
public class ReviewUpdateResponseDto {
    ...
    private Long reviewId;              // 수정된 리뷰 ID
    private String updatedAt;           // 수정 시각 (ISO 8601 format)
}
```
...
### 7.3.4.3 매물 검색 API DTO

#### PropertySearchResultDto (매물 검색 결과)

**패키지**: `com.wherehouse.review.dto`

매물 검색(자동완성) API의 응답에 포함되는 개별 매물 정보.
...
public class PropertySearchResultDto {
    ...
    private String propertyId;          // 매물 식별자 (MD5 Hash)
    private String propertyName;        // 매물명 (아파트명)
    private String address;             // 매물 주소
}
```

**API Endpoint**: `GET /api/v1/properties/search?keyword={keyword}`
...
| DTO | 수정 내용 | 상태 |
|-----|----------|------|
| TopCharterPropertyDto | `reviewCount`, `avgRating` 추가 | 완료 |
| TopMonthlyPropertyDto | `reviewCount`, `avgRating` 추가 | 완료 |
| PropertyDto | `propertyId: Long → String`, 리뷰 필드 추가 | 완료 |

### 리뷰 API DTO (구현 완료, 10개)

| DTO | 용도 | 상태 |
|-----|------|------|
| ReviewCreateRequestDto | 리뷰 작성 요청 | 완료 |
| ReviewCreateResponseDto | 리뷰 작성 응답 | 완료 |
| ReviewListRequestDto | 리뷰 목록 조회 요청 (Query Parameter) | 완료 |
| ReviewListResponseDto | 리뷰 목록 조회 응답 | 완료 |
| FilterMetaDto | 필터링 메타 정보 | 완료 |
| ReviewSummaryDto | 리뷰 요약 (목록용) | 완료 |
| ReviewDetailDto | 리뷰 상세 | 완료 |
| ReviewUpdateRequestDto | 리뷰 수정 요청 | 완료 |
| ReviewUpdateResponseDto | 리뷰 수정 응답 | 완료 |
| PropertySearchResultDto | 매물 검색 결과 | 완료 |
...
| 변수 | 저장 위치 | 갱신 시점 |
|------|----------|----------|
| PositiveCount | `REVIEW_STATISTICS.POSITIVE_KEYWORD_COUNT` | 리뷰 CUD 시 8.4.3 집계 쿼리로 갱신 |
| NegativeCount | `REVIEW_STATISTICS.NEGATIVE_KEYWORD_COUNT` | 리뷰 CUD 시 8.4.3 집계 쿼리로 갱신 |
...
**구현 클래스**: `com.wherehouse.review.service.KeywordExtractor`
...
2. **사전 기반 매칭 (Dictionary Matching)**: 정의된 카테고리별 키워드 사전(`KeywordDictionary`)과 대조하여 매칭되는 항목을 추출한다.
...
#### 8.2.3 키워드 사전 (KeywordDictionary)

**구현 클래스**: `com.wherehouse.review.domain.KeywordDictionary` (Enum)

키워드 추출에 사용되는 사전은 Enum으로 정의되어 있으며, 각 키워드는 카테고리와 기본 점수를 포함한다.
...
public enum KeywordDictionary {
    ...
    KeywordDictionary(String category, int score) {
...
| 카테고리 | 설명 | 긍정 키워드 예시 | 부정 키워드 예시 |
|----------|------|----------------|----------------|
| NOISE | 소음 관련 | 조용 | 시끄럽, 층간소음 |
| LIGHT | 채광 관련 | 밝다, 남향, 채광 | 어둡 |
| TRAFFIC | 교통 관련 | 역세권, 편리, 평지 | 멀다, 불편, 언덕 |
| CONDITION | 시설 상태 | 깨끗 | 노후 |
| SPACE | 공간 관련 | 넓은 | 좁은 |
| COST | 비용 관련 | 관리비저렴 | 관리비비쌈 |
...
**통계 집계 연계**: 저장된 SCORE 값은 8.4.3의 집계 쿼리에서 `SCORE > 0` (긍정) / `SCORE < 0` (부정) 조건으로 카운팅되어 `REVIEW_STATISTICS` 테이블의 `POSITIVE_KEYWORD_COUNT`, `NEGATIVE_KEYWORD_COUNT`로 집계된다. 최종적으로 8.1.3의 키워드 점수 산출에 활용된다.

### 8.3 리뷰 트랜잭션 및 동기화 (Transaction & Synchronization)

**구현 클래스**: `com.wherehouse.review.service.ReviewWriteService`
...
**구현 위치**: `ReviewWriteService` 내부 메서드
...
**메서드명**: `recalculateAndUpdateReviewStatistics(String propertyId)`
...
#### 8.4.3 키워드 통계 재산출 메서드

**메서드명**: `recalculateAndUpdateKeywordStatistics(String propertyId)`
...
8.2.4에서 키워드는 긍정(+1) 또는 부정(-1)의 SCORE 값으로 저장된다. 집계 시 조건부 합계(Conditional Aggregation)를 사용하여 긍정/부정 키워드를 각각 카운팅한다.
...
**구현 위치**: `ReviewWriteService.createReview()` 메서드 내부
...
* **검증 시점**: 리뷰 작성 요청이 ReviewWriteService에 진입한 직후, 트랜잭션 시작 전에 수행한다.
* **검증 메서드**: `ReviewRepository.existsByPropertyIdAndUserId(String propertyId, String userId)`
...
```java
// ReviewWriteService.createReview() 내부
if (reviewRepository.existsByPropertyIdAndUserId(propertyId, userId)) {
    throw new DuplicateReviewException("이미 해당 매물에 대한 리뷰를 작성하셨습니다");
}
```
