...
| 필드 | 타입 | 필수 | 설명 | 동작 방식 |
|----------|------|------|------|----------|
| propertyId | String | ✗ | 매물 이름 | 사용자가 UI에서 매물명을 입력하는 과정에서 발생하는 실시간 키 입력 이벤트마다 별도의 자동완성 API(입력 패턴이 포함된 매물 객체 및 MD5 식별자 리스트 반환)를 호출하여 팝업을 갱신하고, 여기서 사용자가 특정 매물을 선택하면 획득한 MD5 식별자를 기반으로 해당 매물의 리뷰를 조회하며, 입력값이 비어있을 경우 전체 리뷰 목록을 반환한다. |
| page | Integer | ✗ | 페이지 번호 | Default: 1 |
| sort | String | ✗ | 정렬 기준 | rating_desc(최신 작성 날짜, 기본값), rating_asc(가장 먼저 작성한순) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| filterMeta | Object | 필터링 메타 정보 ( 특정 매물 조회인 propertyId 요청 시에만 포함하는 필드) |
...
**Response 예시 (특정 매물 아닌 전체 매물 조회 시)**
...
| userId | String | ✓ | 수정자 ID (작성자 ID 와 동일 해야 됨) |
