...
현재는 데이터 정합성을 가장 확실히 보장할 수 있는 Full Refresh(전체 갱신) 방식을 택했으나, 운영 환경에서 데이터가 수억 건으로 늘어 나는 상황을 가정할 때에 데이터 증가 시 변경된 데이터만 반영하는 Incremental Update(증분 갱신) 방식을 고려 중
...
**구별 인구 데이터:**
- 현재 구현에서는 25개 서울시 구별 인구수가 하드코딩되어 있음으로 추후 이 부분 개선 필요
...
수 많은 위치 데이터를 대상으로 하는 반복적인 **'근처 찾기'** 연산의 성능을 **데이터 양에 거의 영향을 최소화하는**것을 목표로 한다. 이를 위해, 데이터베이스가 가장 효율적으로 처리할 수 있는 B-Tree 인덱스 스캔(Index Scan)으로 설계 하였다.
