...
#### 7.1.1-A PROPERTIES_CHARTER (전세 매물 마스터)

**용도**: 전세 매물의 기준 정보를 저장한다. BatchScheduler에 의해 생성된 불변 ID(MD5)를 PK로 사용하여 데이터의 영속성을 보장한다.
...
| DEPOSIT | NUMBER(15) |  | 전세금 만원 단위 (가변 정보, 배치 시 갱신) |
| DEAL_DATE | VARCHAR2(10) |  | 계약일자 YYYY-MM-DD |
| BUILD_YEAR | NUMBER(4) |  | 건축연도 |
| LEASE_TYPE | VARCHAR2(10) |  | "전세" 고정값 |
| UMD_NM | VARCHAR2(100) |  | 법정동명 |
| ADDRESS | VARCHAR2(200) |  | 전체 주소 (umdNm + jibun) |
| AREA_IN_PYEONG | NUMBER(10,4) |  | 전용면적 평 단위 (excluUseAr × 0.3025) |
| RGST_DATE | VARCHAR2(8) |  | 등록일자 |
| DISTRICT_NAME | VARCHAR2(50) |  | 지역구명 (예: 서울시 강남구) |
| LAST_UPDATED | TIMESTAMP |  | 최종 동기화 일시 |

**인덱스**

* IDX_PROP_CHARTER_MD5: PROPERTY_ID (조회 최적화)
* IDX_PROP_CHARTER_DISTRICT: DISTRICT_NAME (지역구별 조회 최적화)

#### 7.1.1-B PROPERTIES_MONTHLY (월세 매물 마스터)

**용도**: 월세 매물의 기준 정보를 저장한다. BatchScheduler에 의해 생성된 불변 ID(MD5)를 PK로 사용하여 데이터의 영속성을 보장한다.
...
| FLOOR | NUMBER(3) | NOT NULL | 층수 (해시 생성 요소) |
| EXCLU_USE_AR | NUMBER(10,4) | NOT NULL | 전용면적 ㎡ (해시 생성 요소) |
| DEPOSIT | NUMBER(15) |  | 보증금 만원 단위 (가변 정보, 배치 시 갱신) |
| MONTHLY_RENT | NUMBER(10) |  | 월세금 만원 단위 (가변 정보, 배치 시 갱신) |
| DEAL_DATE | VARCHAR2(10) |  | 계약일자 YYYY-MM-DD |
| BUILD_YEAR | NUMBER(4) |  | 건축연도 |
...
* IDX_PROP_MONTHLY_MD5: PROPERTY_ID (조회 최적화)
* IDX_PROP_MONTHLY_DISTRICT: DISTRICT_NAME (지역구별 조회 최적화)
...
#### 7.2.1 매물 상세 정보 (Hash) - 조회 전용 통합 모델 (Read Model)

**[설계 의도 및 데이터 흐름]**

본 데이터 구조는 CQRS 패턴의 'Read Model' 핵심체로서, 성격이 다른 두 종류의 데이터(공공데이터, 사용자 리뷰)를 하나의 객체로 반정규화(Denormalization)하여 저장한다. 이를 통해 추천 서비스(RecommendationService)는 복잡한 DB 조인 연산 없이, O(1)의 속도로 완성된 매물 정보를 조회할 수 있다.

데이터는 두 가지 경로를 통해 이 Hash 구조에 병합(Merge)된다.

**1. 기본 정보 적재 (Batch Path)**:
* **주체**: `RdbSyncListener` (배치)
* **시점**: 매일 새벽 데이터 수집 완료 이벤트 수신 직후.
* **역할**: 국토부 API에서 가져온 매물의 물리적 정보(평수, 층수, 주소)와 가격 정보(`deposit`, `monthlyRent`)를 생성하거나 갱신한다.

**2. 통계 정보 동기화 (Real-Time Path)**:
* **주체**: `ReviewService` (실시간)
* **시점**: 사용자가 리뷰를 작성/수정/삭제하여 트랜잭션이 커밋된 직후 (Write-Through).
* **역할**: RDB에서 계산된 최신 `reviewCount`와 `avgRating` 값만을 이 Hash에 덮어씌워(Partial Update), 배치 주기와 상관없이 항상 최신 통계를 유지한다.

**[데이터 구조 상세 - 전세]**

* **Key Pattern**: `property:charter:{md5_id}`
   * 예: `property:charter:5d41402abc...`

| 필드명 | 데이터 타입 | 설명 | 데이터 원천 (Source) | 갱신 주체 |
|--------|------------|------|---------------------|----------|
| propertyId | String | 매물 식별자 (MD5 Hash 32자) | 배치 생성 | `RdbSyncListener` |
| aptNm | String | 아파트/건물명 | 국토부 API | `RdbSyncListener` |
| excluUseAr | String | 전용면적 ㎡ | 국토부 API | `RdbSyncListener` |
| floor | String | 층수 | 국토부 API | `RdbSyncListener` |
| buildYear | String | 건축연도 | 국토부 API | `RdbSyncListener` |
| dealDate | String | 계약일자 YYYY-MM-DD | 국토부 API | `RdbSyncListener` |
| deposit | String | 전세금 만원 단위 | 국토부 API | `RdbSyncListener` |
| leaseType | String | "전세" 고정값 | 국토부 API | `RdbSyncListener` |
| umdNm | String | 법정동명 | 국토부 API | `RdbSyncListener` |
| jibun | String | 지번 | 국토부 API | `RdbSyncListener` |
| sggCd | String | 시군구코드 | 국토부 API | `RdbSyncListener` |
| address | String | 전체 주소 (umdNm + jibun) | 국토부 API | `RdbSyncListener` |
| areaInPyeong | String | 전용면적 평 단위 | 국토부 API | `RdbSyncListener` |
| rgstDate | String | 등록일자 | 국토부 API | `RdbSyncListener` |
| districtName | String | 지역구명 | 국토부 API | `RdbSyncListener` |
| reviewCount | String | 총 리뷰 개수 | RDB (REVIEW_STATISTICS) | `ReviewService` |
| avgRating | String | 평균 별점 | RDB (REVIEW_STATISTICS) | `ReviewService` |

**[데이터 구조 상세 - 월세]**

* **Key Pattern**: `property:monthly:{md5_id}`
   * 예: `property:monthly:5d41402abc...`
...
| buildYear | String | 건축연도 | 국토부 API | `RdbSyncListener` |
| dealDate | String | 계약일자 YYYY-MM-DD | 국토부 API | `RdbSyncListener` |
| deposit | String | 보증금 만원 단위 | 국토부 API | `RdbSyncListener` |
| monthlyRent | String | 월세금 만원 단위 | 국토부 API | `RdbSyncListener` |
| leaseType | String | "월세" 고정값 | 국토부 API | `RdbSyncListener` |
| umdNm | String | 법정동명 | 국토부 API | `RdbSyncListener` |
| jibun | String | 지번 | 국토부 API | `RdbSyncListener` |
| sggCd | String | 시군구코드 | 국토부 API | `RdbSyncListener` |
| address | String | 전체 주소 (umdNm + jibun) | 국토부 API | `RdbSyncListener` |
...
1. **데이터 타입 통일**: 기존 시스템과의 호환성을 위해 모든 필드를 **String 타입**으로 저장한다. 숫자 필드(deposit, floor 등)도 `toString()` 변환 후 저장한다.

2. **Null 처리**: 배치 적재 시점에는 리뷰가 없을 수 있으므로, `reviewCount`와 `avgRating`이 Null일 경우 각각 `"0"`과 `"0.0"`으로 초기화하여 적재해야 한다.

3. **부분 업데이트**: `ReviewService`는 리뷰 작성 후 Redis 동기화 시, 전체 Hash를 다시 쓰지 않고 `HSET` 명령을 사용하여 통계 필드(`reviewCount`, `avgRating`)만 효율적으로 갱신해야 한다.

4. **Key Pattern 유지**: 기존 추천 서비스와의 호환성을 위해 `property:charter:{ID}`, `property:monthly:{ID}` 키 패턴을 반드시 유지해야 한다.

#### 7.2.2 검색 인덱스 (Sorted Set) - 레거시 호환성 보장 (Legacy Compatibility)

**[설계 의도 및 데이터 흐름]**

본 시스템은 이벤트 기반 아키텍처(Event-Driven Architecture)로 전환됨에 따라 데이터 적재의 주체와 흐름이 다음과 같이 변경된다.

* **AS-IS (기존)**: `BatchScheduler`가 데이터 수집 후 Redis에 직접 적재.
* **TO-BE (변경)**:
   1. `BatchScheduler`: 데이터 수집 및 MD5 ID 생성 후 이벤트만 발행 (Redis 적재 로직 제거).
   2. `RdbSyncListener`: 이벤트 수신 → RDB(Master) 적재(Upsert) → Redis(Read Model) 동기화.

이 구조에서 `RdbSyncListener`는 Redis에 데이터를 적재할 때, 기존 추천 서비스(`RecommendationService`)가 참조하고 있는 인덱스 키 패턴을 100% 동일하게 재현해야 한다. 이는 배치 아키텍처 변경 후에도 실시간 추천 서비스가 중단 없이 작동하도록 하위 호환성(Backward Compatibility)을 보장하기 위한 필수 계약(Contract)이다.

**[인덱스 정의 및 Key Pattern]**

| 인덱스 종류 | Redis Key 패턴 (Literal) | 자료구조 | Score 기준 | 비고 |
|------------|-------------------------|---------|-----------|------|
| 전세금 인덱스 | `idx:charterPrice:{districtName}` | ZSet | 전세금(만원) | 전세 가격 필터링용 (AS-IS 유지) |
| 보증금 인덱스 | `idx:deposit:{districtName}` | ZSet | 보증금(만원) | 월세 매물의 보증금 필터링용 |
| 월세금 인덱스 | `idx:monthlyRent:{districtName}:월세` | ZSet | 월세금(만원) | 월세 매물의 월세금 필터링용<br>(주의: 접미사 `:월세` 필수) |
| 전세 평수 인덱스 | `idx:area:{districtName}:전세` | ZSet | 전용면적(평) | 전세 평수 필터링용 |
| 월세 평수 인덱스 | `idx:area:{districtName}:월세` | ZSet | 전용면적(평) | 월세 평수 필터링용 |

**[구현 제약 사항]**

1. **Key Naming**: 위 표의 `{districtName}`은 '서울시' 접두어를 제외한 자치구명 (예: '강남구', '종로구')만을 사용해야 한다.
2. **Data Type**: 모든 Score 값은 `Double` 타입으로 저장하며, Member 값은 `String` 타입의 MD5 Hash ID를 사용한다.
3. **동기화 시점**: `RdbSyncListener`는 RDB에 매물 데이터가 성공적으로 적재(Upsert)된 후, 즉시 위 인덱스들에 대해 `ZADD` 명령을 수행하여 실시간 검색 가능 상태로 만든다.

### 7.3 DTO 구조

API 통신에 사용되는 주요 데이터 전송 객체 정의.

#### 7.3.1 PropertyDto (Response)

**[Critical Change] 식별자 타입 변경 (Migration Required)**

기존 시스템의 `PropertyDto`는 `Long propertyId`를 사용하고 있으나, 데이터 영속성을 위한 MD5 해시(32자) 도입에 따라 이를 `String` 타입으로 변경해야 한다. 이는 기존 클라이언트와의 호환성을 깨뜨리는 변경이므로 API 버전 관리(v1) 적용 시 필수 점검 사항이다.

**수정된 DTO 구조 정의**

```java
public class PropertyDto {
    // [변경] MD5 Hash(32자) 수용을 위해 Long -> String으로 타입 변경 필수
    // AS-IS: private Long propertyId;
    // TO-BE: private String propertyId;  (예: "5d41402abc4b2a76b9719d911017c592")
    ...
    // [신규 추가] 리뷰 시스템 연동 필드
    private Integer reviewCount;     // 리뷰 개수 (통계)
    private Double avgRating;        // 평균 별점 (통계)
    private Double finalScore;       // 최종 추천 점수 (Legacy 70% + Review 30%)
}
```
...
#### 7.3.3 추천 결과 DTO (TopPropertyDto 계열) - 리뷰 정보 확장

사용자에게 추천된 매물 목록을 보여줄 때, 단순 매물 정보뿐만 아니라 **리뷰 요약 정보(평점, 개수)**를 함께 노출하여 클릭률을 높이고 신뢰도를 제공해야 한다. 이를 위해 기존 전세/월세 추천 결과 DTO에 관련 필드를 추가한다.

**1. 전세 추천 결과 (TopCharterPropertyDto)**

```java
public class TopCharterPropertyDto {
    private String propertyId;       // MD5 Hash ID
    private String propertyName;     // 아파트명
    ...
    // [신규 추가] 리뷰 요약 정보
    // Redis의 'property:charter:{id}' 해시에서 조회하여 매핑
    private Integer reviewCount;     // 총 리뷰 수
    private Double avgRating;        // 평균 별점
}
```

**2. 월세 추천 결과 (TopMonthlyPropertyDto)**
...
public class TopMonthlyPropertyDto {
    ...
    // [신규 추가] 리뷰 요약 정보
    // Redis의 'property:monthly:{id}' 해시에서 조회하여 매핑
    private Integer reviewCount;     // 총 리뷰 수
    private Double avgRating;        // 평균 별점
}
```
...
* **Data Mapping**: RecommendationService는 후보 매물 ID를 추출한 후, Redis Pipeline을 통해 `property:{type}:{id}` 해시 데이터를 조회한다. 이때 함께 반환된 `reviewCount`와 `avgRating` 값을 위 DTO의 해당 필드에 매핑해야 한다.

* **Null Safety**: 만약 Redis에 해당 필드가 없거나 Null인 경우, 0 (0건, 0.0점)으로 초기화하여 클라이언트에 전달한다.
...
* **LegacyScore**: 위 8.1.2항에 따라 산출된 기존 정량적 점수 (100점 만점).
* **ReviewScore**: 사용자 평균 별점을 100점 척도로 정규화한 값.
   * 공식: `(AverageRating / 5.0) * 100`
* **예외 처리**: 리뷰 개수가 통계적 유의성 기준(5개) 미만일 경우, 리뷰 가중치를 0으로 하고 `LegacyScore`를 100% 반영한다.

#### 8.1.4 예외 처리 (Cold Start 대응)
