...
| DEPOSIT | NUMBER(15) |  | 전세금 (가변 정보, 배치 시 갱신) |
...
| MONTHLY_RENT | NUMBER(10) |  | 월세금 (가변 정보, 배치 시 갱신) |
| LAST_UPDATED | TIMESTAMP |  | 최종 동기화 일시 |
...
* **Key Pattern**: `property:{leaseType}:{md5_id}`
   * 예: `property:charter:5d41402abc...` (전세), `property:monthly:5d41402abc...` (월세)

| 필드명 | 설명 | 데이터 원천 (Source) | 갱신 주체 |
|--------|------|---------------------|----------|
| propertyId | 매물 식별자 (MD5 String) | 배치 생성 | `RdbSyncListener` |
| aptNm | 아파트/건물명 | 국토부 API | `RdbSyncListener` |
| address | 전체 주소 | 국토부 API | `RdbSyncListener` |
| deposit | 보증금/전세금 (Integer) | 국토부 API | `RdbSyncListener` |
| monthlyRent | 월세금 (Integer) | 국토부 API | `RdbSyncListener` |
| excluUseAr | 전용면적 (Double) | 국토부 API | `RdbSyncListener` |
| reviewCount | 총 리뷰 개수 (Integer) | RDB (REVIEW_STATISTICS) | ReviewService |
| avgRating | 평균 별점 (Double) | RDB (REVIEW_STATISTICS) | ReviewService |

**[구현 제약 사항]**

1. **Null 처리**: 배치 적재 시점에는 리뷰가 없을 수 있으므로, `reviewCount`와 `avgRating`이 Null일 경우 각각 `0`과 `0.0`으로 초기화하여 적재해야 한다.
2. **부분 업데이트**: `ReviewService`는 리뷰 작성 후 Redis 동기화 시, 전체 Hash를 다시 쓰지 않고 `HSET` 명령을 사용하여 통계 필드(`reviewCount`, `avgRating`)만 효율적으로 갱신해야 한다.
