...
window.onload = function () {
    ...
    menu_property_icon = document.getElementById("menu_property_icon");
    iframeSection = document.getElementById("iframe_section");

    initIframe();

    menu_suggest_icon.addEventListener("click", () => clickMenu(1));
    menu_property_icon.addEventListener("click", () => clickMenu(2));
    menu_detail_icon.addEventListener("click", () => clickMenu(3));
    menu_review_icon.addEventListener("click", () => clickMenu(4));
    menu_gu_icon.addEventListener("click", () => clickMenu(5));
    menu_board_icon.addEventListener("click", () => clickMenu(6));

    function clickMenu(sel) {
        menu_suggest_icon.style.backgroundColor = "rgba(11, 94, 215, 1)";
        menu_property_icon.style.backgroundColor = "rgba(11, 94, 215, 1)";
        ...
        if (sel === 1) {
            ...
            iframeSection.src = "/wherehouse/properties/board";
        } else if (sel === 3) {
...
function initIframe() {
    ...
    if (iframe_target === "house_rec") {
        ...
        menu_property_icon.style.backgroundColor = "rgba(34, 34, 34, 0.3)";
        iframe_target = "/wherehouse/properties/board";
