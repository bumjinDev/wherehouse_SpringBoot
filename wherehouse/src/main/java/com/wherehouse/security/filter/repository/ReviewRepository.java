...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    //
    // [주의사항]
    // JPQL Bulk DELETE는 영속성 컨텍스트를 거치지 않고 DB에 직접 쿼리한다.
    // 호출 시점에 영속성 컨텍스트에 해당 엔티티가 로드되어 있으면
    // DB와 메모리 사이에 불일치가 발생할 수 있다.
    // 이 메서드는 BanService에서만 호출되며, BanService는 Review 엔티티를
    // 영속성 컨텍스트에 로드하지 않으므로 clearAutomatically는 설정하지 않았다.
    //
    // [실행되는 SQL]
