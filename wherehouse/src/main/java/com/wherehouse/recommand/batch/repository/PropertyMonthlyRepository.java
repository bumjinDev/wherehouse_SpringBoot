 ...
 * 월세 매물 데이터 접근 객체
 *
 * 역할: PROPERTIES_MONTHLY 테이블에 대한 CRUD 수행
...
public interface PropertyMonthlyRepository extends JpaRepository<PropertyMonthly, String> {
     ...
     */
    List<PropertyMonthly> findByRgstDate(String rgstDate);
     ...
     *
     * [배치 처리에서 Slice 선택 이유]
     * 1. COUNT 쿼리 제거: 대용량 테이블(56,272건)에서 COUNT(*)는 Full Table Scan 유발
     ...
     *
     * 생성되는 쿼리 예시 (Oracle):
     * SELECT * FROM PROPERTIES_MONTHLY
     ...
     * Slice<PropertyMonthly> slice;
     *
     * do {
     *     slice = propertyMonthlyRepository.findAllBy(pageable);
     *     List<PropertyMonthly> chunk = slice.getContent();
     *
     *     // 청크 단위 처리 로직
     *     processChunk(chunk);
     *
     ...
     */
    Slice<PropertyMonthly> findAllBy(Pageable pageable);
}
