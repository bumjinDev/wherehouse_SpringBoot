...
public class MembersRepository implements IMembersRepository {
     ...
     */
    @Override
    public int addMember(MembersVO membersVO, UserEntity userEntity) {
        memberEntityRepository.save(membersVO); // 회원 관리 테이블에 데이터 저장
     ...
     */
    @Override
    public int editMember(MembersVO memberVO, UserEntity userEntity) {
        // 닉네임 중복 확인: 닉네임이 기존과 다르고 다른 사용자와 중복되지 않을 경우에만 수정 가능
        if (!memberEntityRepository.findByNicknameAndNotIdNative(memberVO.getNickName(), memberVO.getId()).isPresent()) {
            memberEntityRepository.save(memberVO); // 회원 관리 테이블 데이터 수정
