...
- 반경 500m 내 사용자가 지정한 핀 포인트를 기준으로 해당 지역구 내 CCTV 위치에 마커를 생성
- CCTV 개수 확인
- 가장 가까운 파출소 거리 확인
- 편의성 및 안정성 점수 확인.
...
---
# 개선 목표
1. 기존 브라우저 로드 시 마다 필요한 JSON 파일 데이터를 매 순간 가져옴으로 확인되는 지연 현상을 해결 하기 위해 JSON 데이터를 DB 에 저장 후 이를 Reduis 캐싱 기술 적용하여 요청 시 마다 JSON 데이터 전체가 아닌 요청에 필요한 데이터만 부분적으로 로드.
2. 테스트 코드 작성: JUnit을 활용해 테스트 코드를 작성하고, 이를 Jenkins와 연계하여 지속적 통합(CI) 환경 구축.
3. AOP 적용하여 로그 기능 개발
4. React 적용
5. 코드 리팩토링: 기존 코드를 구조적으로 개선하여 가독성과 유지보수성을 강화.
6. SQL Injection 방지 적용: 안전한 데이터 처리를 위해 입력 값 검증.
