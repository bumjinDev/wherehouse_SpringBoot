...
Oracle이 SSOT(쓰기 원천·ACID)로 매물·리뷰·예약 원본을 보관한다. Redis는 읽기 가속용으로, 추천 후보 검색을 위한 Sorted Set 인덱스와 상세 위치 분석의 2단계 응답 캐시(지도)를 담는다. 쓰기는 항상 Oracle에 먼저 반영되고 동기화가 그 결과를 Redis 읽기 모델로 전파한다 — 사용자 매물 등록이 갱신한 인덱스를 추천이 그대로 읽는 결합이 그 예다. 다만 모든 읽기가 Redis를 경유하지는 않으며, 통합 목록 조회처럼 Oracle을 직접 조회하는 경로도 있다.

### 외부 연동 · 배치

외부로는 카카오맵 API(좌표→주소·편의시설)와 국토교통부 실거래가 API(매물 수집)를 연동한다. 요청 경로와 분리된 백그라운드 배치가 실거래가 수집·적재, RDB→Redis 동기화, 추천 인덱스 재생성, 만료 슬롯 정리를 주기적으로 수행한다(주기는 위 그림 참조).

### 처리 흐름: 주거지 추천 폴백 되먹임 루프

1차 검색은 가격·평수 조건을 함께 만족하는 매물을 추린 뒤, 실제 값으로 필수 조건을 다시 확인한다. 자치구별 통과 매물이 최소 기준에 못 미치면 우선순위가 낮은 조건부터 완화해 부족한 자치구만 다시 검색하고, 그래도 부족하면 한 단계 더 완화한다. 이후 정량 점수와 리뷰 평가를 합산한 하이브리드 점수로 자치구를 순위화해 응답한다.
...
예약은 먼저 슬롯과 윈도우를 잠금 없이 조회해 대상을 확인한 뒤, 시작 시각 만료·자기 매물·동일 매물 중복·시간대 겹침을 차례로 차단한다. 슬롯이 예약 가능한지는 미리 검사하지 않는다 — 예약 가능 상태일 때만 점유하는 단일 처리로, 먼저 점유에 성공한 한 명만 확정하고 경합에서 밀린 요청은 거부한다. 사전 검증을 통과해도 최종 확정은 이 점유 결과로 갈린다.
...
매물 등록은 단순 직렬 흐름이라 별도 다이어그램을 두지 않는다: 불변 속성을 MD5로 해시해 식별자를 만들고, Oracle PK 유니크 제약으로 동일 매물의 동시 등록을 차단한 뒤 저장하고, 커밋 직후 위의 등록 인덱스 동기화를 수행한다.
