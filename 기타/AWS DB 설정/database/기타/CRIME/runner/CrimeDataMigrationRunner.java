...
public class CrimeDataMigrationRunner implements CommandLineRunner {
    ...
    private final SourceCrimeRepository sourceRepository;
    private final DestinationCrimeRepository destinationRepository;
    ...
    public void run(String... args) {
        ...
        try {
            // 1. Destination DB에 데이터가 이미 있는지 확인
            ...
            // 2. Source DB에서 모든 데이터 조회
            List<AnalysisCrimeStatistics> sourceData = sourceRepository.findAll();
            ...
            List<AnalysisCrimeStatistics> newDataToSave = sourceData.stream()
                    .map(sourceEntity -> AnalysisCrimeStatistics.builder()
                            .districtName(sourceEntity.getDistrictName())
                            .year(sourceEntity.getYear())
                            .totalOccurrence(sourceEntity.getTotalOccurrence())
                            .totalArrest(sourceEntity.getTotalArrest())
                            .murderOccurrence(sourceEntity.getMurderOccurrence())
                            .murderArrest(sourceEntity.getMurderArrest())
                            .robberyOccurrence(sourceEntity.getRobberyOccurrence())
                            .robberyArrest(sourceEntity.getRobberyArrest())
                            .sexualCrimeOccurrence(sourceEntity.getSexualCrimeOccurrence())
                            .sexualCrimeArrest(sourceEntity.getSexualCrimeArrest())
                            .theftOccurrence(sourceEntity.getTheftOccurrence())
                            .theftArrest(sourceEntity.getTheftArrest())
                            .violenceOccurrence(sourceEntity.getViolenceOccurrence())
                            .violenceArrest(sourceEntity.getViolenceArrest())
                            .build())
